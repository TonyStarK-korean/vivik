...
class OneMinuteSurgeEntryStrategy:
    ...
    def __init__(self, api_key=None, secret_key=None, sandbox=False):
        ...
        if not sandbox:
            try:
                ...
                if ws_started:
                    ...
                    print("🎯 동적 WebSocket subscription 시스템 Active화됨")
                ...
                self.logger.error(f"❌ WebSocket 시스템 Initialization failed: {e}")
        ...
        if HAS_TRADE_HISTORY_SYNC and self.exchange:
            try:
                ...
                self.logger.info("📊 Trade 내역 Sync 시스템 Initialization complete")
            except Exception as e:
                self.logger.error(f"Trade 내역 Sync 시스템 Initialization failed: {e}")
                self.trade_history_sync = None
        ...
        self.logger.info("🛡️ Rate Limit 추적 시스템 Initialization complete (per minute 1200 weight)")
        ...
        if HAS_ORDER_HISTORY_SYNC and self.exchange and hasattr(self.exchange, 'apiKey') and self.exchange.apiKey:
            try:
                ...
                self.logger.info("📊 주문 기록 Sync 시스템 Initialization complete")
            except Exception as e:
                self.logger.error(f"주문 기록 Sync 시스템 Initialization failed: {e}")
                self.order_history_sync = None
    ...
    def update_websocket_kline(self, symbol, timeframe, kline_data):
        ...
        try:
            ...
            self.logger.error(f"웹소켓 kline 데이터 Update Failed ({symbol}, {timeframe}): {e}")
                ...
                if hasattr(self, '_api_rate_limited') and self._api_rate_limited:
                    ...
                    for future in as_completed(future_to_timeframe):
                        ...
                        try:
                            ...
                            if hasattr(self, '_api_rate_limited') and self._api_rate_limited:
                                self.logger.warning(f"🚨 Rate Limit 감지 - 병렬 조times 중단")
                                break
                            ...
                            if ("418" in str(e) or "429" in str(e) or 
                                ...
                                self.logger.error(f"🚨 병렬 조times 중 Rate Limit 감지: {symbol} {timeframe} - {e}")
                                self._api_rate_limited = True
                                self._last_rate_limit_check = time.time()
                ...
                try:
                    ...
                    if df_3m is not None and len(df_3m) >= 100:
                        ...
                        if isinstance(result, tuple) and len(result) == 2:
                            ...
                            if status:
                                print(f"✅ [{clean_symbol}] Entry 조건 만족: {status}")
                            else:
    ...
    def _force_preload_websocket_buffer(self, symbol, timeframe, limit=1000):
        ...
        try:
            ...
            if self._api_rate_limited:
                ...
                # 전략에 Required한 Timeframe만 Subscription (전략A 비Active화로 1m Excluded, 4h는 REST API Filtering 전용)
        ...
        try:
            ...
            if to_subscribe:
                ...
                for batch_idx in range(total_batches):
                    ...
                    try:
                        # Batch Subscription (4h Excluded - REST API Filtering 전용)
    ...
    def _subscribe_initial_major_symbols(self):
        ...
        try:
            ...
            print(f"   💾 버퍼 Initialization complete")
    ...
    def check_5m_supertrend_entry_signal(self, symbol, df_5m):
        ...
        try:
            ...
            if 'supertrend_direction' in df_5m_calc.columns:
                ...
                self.logger.debug(f"No SuperTrend column - 조건 우times: {symbol}")
                return True
            ...
            if 'supertrend' in df_5m_calc.columns:
                ...
                self.logger.debug(f"SuperTrend 조건3 Skip ({clean_symbol}): supertrend Column Absent")
            
            self.logger.debug(f"SuperTrend 5봉조건 Failed ({clean_symbol}): All conditions not met")
            return False
        except Exception as e:
    ...
    def check_high_surge_conditions(self, symbol, df_1m, change_24h):
        ...
        try:
            ...
            self.logger.error(f"급등 조건 체크 Failed ({symbol}): {e}")
        ...
        try:
            ...
            if strategy_3m_2nd_enabled and df_3m is not None and df_1d is not None:
                try:
                    ...
                    #     print(f"🔍 [DEBUG] {symbol.replace('/USDT:USDT', '')}: 3minute candles2번째 미충족 {missing_conditions}count (관심종목 후보!)")
            ...
            if "418" in str(e) or "too many requests" in str(e).lower():
                ...
                self.logger.debug(f"{symbol} Entry 조건 체크 건너뛰기 (Rate limit)")
                return False, [f"Rate limit - 조건 체크 건너뛰기"]
            else:
                self.logger.error(f"{symbol} Strategy C/D Entry 조건 체크 Failed: {e}")
    ...
    def _print_near_entry_signals(self, near_entry):
        ...
        if near_by_symbol:
            ...
            for strategy_type in strategy_order:
                print(f"\nEntry임박 [{strategy_type}] (1count 조건 미충족)")
                if strategy_type not in near_groups:
                    print("   Absent")
                    continue
                ...
                for symbol, result in symbol_results:
                    ...
                    if strategy_type == 'Strategy D: 5minute candles 초입 초강력 타점':
                        ...
                        if failed_count == 1:
                            print(f"   \033[93m\033[1m{clean_symbol}\033[0m [{change_pct:+.1f}%] ({passed_count}/{total_conditions}) ⚠️ 1count 조건 미충족")
                        else:
                            print(f"   {clean_symbol} [{change_pct:+.1f}%] ({passed_count}/{total_conditions}) ⚠️ {failed_count}count 조건 미충족")
            ...
            print(f"\nEntry임박 [Strategy C: 3minute candles 시세 초입 포착] (1count 조건 미충족)")
            print("   Absent")
            print(f"\nEntry임박 [Strategy D: 5minute candles 초입 초강력 타점] (1count 조건 미충족)")
            print("   Absent")
    ...
    def _print_potential_entry_signals(self, potential_entry):
        ...
        if potential_entry:
            ...
            strategy_order = ['Strategy C+D: 3minute candles+5minute candles 복합 Entry', 'Strategy C: 3minute candles 시세 초입 포착', 'Strategy D: 5minute candles 초입 초강력 타점']
            for strategy_type in strategy_order:
                print(f"\nEntry확률 [{strategy_type}] (2count 조건 미충족)")
                if strategy_type not in potential_groups:
                    print("   Absent")
                    continue
                ...
                for result in results:
                    ...
                    print(f"   {clean_symbol} [{change_24h:+.1f}%] ({passed_count}/{total_conditions}) ⚠️ {failed_count}count 조건 미충족")
                    # 미충족 조건들을 상세히 표시
                    for failed_msg in failed_msgs:
                        print(f"\033[91m      ❌ {failed_msg}\033[0m")
        else:
            # 모든 전략을 count별적으로 표시 (C전략 → D전략 순서)
            print(f"\nEntry확률 [Strategy C: 3minute candles 시세 초입 포착] (2count 조건 미충족)")
            print("   Absent")
            print(f"\nEntry확률 [Strategy D: 5minute candles 초입 초강력 타점] (2count 조건 미충족)")
            print("   Absent")
    ...
    def _print_watchlist_signals(self, watchlist):
        ...
        if watchlist:
            ...
            for strategy in strategy_order:
                ...
                print(f"\n[WATCHLIST] 관심종목 [{strategy}] ({avg_failed:.0f}count 조건 미충족, 평균 {avg_total-avg_failed:.0f}/{avg_total:.0f} 통과)")
            ...
            if failed_condition_stats:
                print(f"\n" + "="*60)
                print(f"📊 관심종목 미충족 조건 통계 (상위 10count)")
                print(f"="*60)
            ...
            print(f"\n[WATCHLIST] 관심종목 (3~4count 조건 미충족)")
            print("   Absent")
        ...
        if hasattr(self, '_speed_test_mode') and self._speed_test_mode:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                if cache_size >= expected_cache_entries * 0.8:
                    ...
                    print(f"🚀 WebSocket 모드: {ws_buffer_count}count 버퍼 → 고속 (<10초)")
        ...
        if entry_signals:
            for result in entry_signals:
                ...
                if hasattr(self, '_trading_disabled') and self._trading_disabled:
                    print(f"[속도Test] ⚡ {clean_symbol} 매매 Execute 건너뛰기 (속도 우선)")
                    continue
    ...
    def execute_trade(self, symbol, entry_price):
        ...
        try:
            ...
            if self.dca_manager:
                try:
                    ...
                    if dca_result and dca_result.get('success'):
                        ...
                        print(f"[DCAEntry] ⚠️ DCA 시스템 Entry Failed, Legacy 방식으로 전환")
                        # DCA Failed 시 Legacy 방식으로 fallback
                        
                except Exception as e:
                    print(f"[DCAEntry] ❌ DCA 시스템 Error: {e}")
            ...
            if order and order.get('id'):
                ...
                if self.dca_manager:
                    try:
                        print(f"[DCA변환] 🔄 Legacy 방식 Entry → DCA 시스템 변환 Attempt...")
                        ...
                        if conversion_result:  # DCAPosition 객체가 반환되면 Success
                            print(f"[DCA변환] ✅ DCA 시스템 변환 Success - 1차/2차 DCA 주문 자동 Batch")
                ...
                if self.dca_manager:
                    ...
                    if clean_symbol in self.dca_manager.positions:
                        ...
                        if profit_pct <= stop_loss_pct:
                            ...
                            self.logger.critical(f"🚨 손절 트리거: {symbol} - Stage: {current_stage}, Profit ratio: {profit_pct:.2f}%, 손절선: {stop_loss_pct:.1f}%")
                    ...
                    if profit_pct <= stop_loss_pct:
                        ...
                        self.logger.critical(f"🚨 손절 트리거: {symbol} - Profit ratio: {profit_pct:.2f}%, 손절선: {stop_loss_pct:.1f}%")
        ...
        try:
            ...
            try:
                ...
                if self.dca_manager:
                    ...
                    print(f"⚠️ {clean_symbol}: DCA 시스템 비Active화 - Entry Skip")
        ...
        try:
            ...
            if is_dca_managed and self.dca_manager:
                try:
                    ...
                    print(f"[Exit요청] 📋 {clean_symbol} → DCA 시스템 Exit 요청")
                        ...
                        if partial_ratio >= 1.0:
                            ...
                            if self.dca_manager and hasattr(self.dca_manager, 'notify_liquidation_from_strategy'):
                                try:
                                    ...
                                    print(f"[Sync] 🔄 {clean_symbol} DCA 시스템에 Exit Complete 통지")
            ...
            if order and order.get('id'):
                ...
                if partial_ratio >= 1.0:  # 전량 Exit
                    ...
                    if self.dca_manager and hasattr(self.dca_manager, 'notify_liquidation_from_strategy'):
                        try:
                            ...
                            print(f"[Sync] 🔄 {clean_symbol} DCA 시스템에 Exit Complete 통지 (fallback)")
                        except Exception as sync_error:
                            print(f"[Sync] ⚠️ {clean_symbol} DCA Exit 통지 Failed: {sync_error}")
                ...
                if symbol in self.partial_exit_accumulator:
                    ...
                    self.logger.info(f"📊 부분Exit Cumulative 데이터 정리 Complete (Legacy방식): {clean_symbol}")
            ...
            try:
                ...
                if exchange_positions:
                    ...
                    if recovery_result.get('successful_recoveries', 0) > 0:
                        ...
                        self.logger.debug(f"DCA 누락 감지: {len(recovery_result['missing_orders_detected'])}count 주문")
                    elif recovery_result.get('predictive_placements'):
                        self.logger.debug(f"DCA 예측 Batch: {len(recovery_result['predictive_placements'])}건")
                    else:
        ...
        if (hasattr(self, 'trade_history_sync') and self.trade_history_sync and
            ...
            try:
                ...
                if sync_result.get('new_trades_found', 0) > 0:
                    print(f"[TradeSync] ✅ {sync_result['new_trades_found']}건 New Trade 발견 및 Sync")
        ...
        if current_time - self._last_cyclic_check_time > 5:  # 5초
            try:
                if (self.dca_manager and hasattr(self.dca_manager, 'monitor_cyclic_opportunities') and
                    ...
                    if opportunities:
                        print(f"[Cyclic trading] 🔄 {len(opportunities)}count 기times 감지")
                        ...
                        if execution_result['executed'] > 0:
                            print(f"[Cyclic trading] ✅ {execution_result['executed']}건 부분Exit Execute Complete")
                            ...
                            print(f"[Cyclic trading] ⚠️ {failed_count}건 Execute Failed")
                        ...
                        if dca_position.is_active:
                            ...
                            if current_price:
                                ...
                                if current_price <= stop_loss_price:
                                    ...
                                    print(f"[실Time손절] 🚨 {clean_symbol} 손절 감지 ({dca_position.current_stage}Stage: -{stop_loss_pct_display:.0f}%): Current price ${current_price:.6f} <= 손절가 ${stop_loss_price:.6f}")

                                    # 즉시 손절 Execute
                                    exit_reason = f"평균가 -{stop_loss_pct_display:.0f}% 손절 ({dca_position.current_stage})"
                                    if self.execute_exit_trade(symbol, exit_reason, partial_ratio=1.0):
                                        print(f"[실Time손절] ✅ {clean_symbol} 손절 Complete")
                                    else:
                                        print(f"[실Time손절] ❌ {clean_symbol} 손절 Failed")
            except Exception as stop_loss_error:
                print(f"[실Time손절] ❌ 손절 감지 Failed: {stop_loss_error}")
        ...
        if current_time - self._last_new_exit_check > 2:  # 2초마다 체크
            if self.dca_manager and self.active_positions:
                try:
                    for symbol in list(self.active_positions.keys()):
                        ...
                        if current_price:
                            ...
                            if exit_signal:
                                ...
                                print(f"[Exit신호] 🎯 {clean_symbol} {exit_type} 감지: {trigger_info}")
                    ...
                    self.logger.error(f"New Exit 조건 체크 Failed: {e}")
            ...
            try:
                for symbol in list(self._emergency_exit_requests):
                    ...
                    if self.execute_exit_trade(symbol, "API밴 대응 긴급Exit", partial_ratio=1.0):
                        ...
                        print(f"[긴급Exit] ❌ {clean_symbol} Exit Failed - 재Attempt 대기")
                ...
                try:
                    ...
                    if is_dca_managed and self.dca_manager:
                        ...
                        try:
                            ...
                            print(f"[DCA모니터링] ⚠️ DCA 시스템 조times Failed: {e}")
        ...
        try:
            ...
            if event_type and data:
                if event_type == "dca_entry":
                    ...
                    if symbol in self.partial_exit_accumulator:
                        del self.partial_exit_accumulator[symbol]
                        self.logger.info(f"📊 부분Exit Cumulative 데이터 정리 Complete: {symbol}")
        ...
        try:
            ...
            if hasattr(self, '_api_rate_limited') and self._api_rate_limited:
                ...
                if local_count != dca_count:
                    ...
                    print(f"   시스템 Recover 후 수동으로 Position을 Confirm해주세요.")
            ...
            try:
                ...
                if any(pos['contracts'] > 0 for pos in positions):
                    ...
                    print(f"[PositionSync] 📊 Trade소로부터 {len(positions)}count Position 데이터 수신")
            except Exception as e:
                # API 에러 Process (Rate Limit 감지 포함)
                error_str = str(e).lower()
                if ("418" in str(e) or "429" in str(e) or 
                    "too many requests" in error_str or "rate limit" in error_str):
                    print(f"[PositionSync] 🚨 Rate Limit 감지 - API calls 차단: {e}")
                    self._api_rate_limited = True
                    self._last_rate_limit_check = time.time()
                    ...
                    if hasattr(self, 'active_positions') and self.active_positions:
                        position_count = len(self.active_positions)
                        print(f"📊 [계좌Position] 보유중: {position_count}count (API 에러로 인한 Cache Info)")
                        for symbol, position in self.active_positions.items():
                            clean_symbol = symbol.replace('/USDT:USDT', '').replace('/USDT', '')
                            entry_price = position.get('entry_price', 0)
                            print(f"   🔹 {clean_symbol}: ${entry_price:.6f} (Cache)")
                    else:
                        print(f"📊 [계좌Position] 보유중: Absent (API 에러)")
                    return
                else:
            ...
            if symbol in self.dca_manager.positions:
                ...
                if dca_position.is_active:
                    ...
                    if price_diff_pct > 0.05:  # 0.05% 이상 차이
                        print(f"[DCASync] ⚠️ {symbol.replace('/USDT:USDT', '')} 평균가 차이 감지:")
                ...
                self.logger.debug(f"[DCASync] ℹ️ {symbol.replace('/USDT:USDT', '')} DCA 시스템에서 관리되지 않음")
    ...
    def _handle_removed_positions(self, active_exchange_positions, sync_stats):
        ...
        if positions_to_remove:
            ...
            print(f"[RemovePosition] ✅ {len(positions_to_remove)}count Position 정리 Complete")
    ...
    def _generate_sync_summary_report(self, sync_stats, detailed_reports, quiet=False):
        ...
        if any([sync_stats['price_corrections'], sync_stats['dca_corrections'], 
            ...
            if sync_stats['quantity_corrections'] > 0:
                print(f"   📊 Quantity 교정: {sync_stats['quantity_corrections']}건")
        ...
        if detailed_reports['dca_sync']:
            ...
            if sync_stats['validation_errors'] > 0:
                print(f"\n⚠️ Verification Error: {sync_stats['validation_errors']}건")
    ...
    def _full_4h_filtering(self, candidate_symbols):
        ...
        filtered_symbols = []
        batch_size = 50  # 🚀 최적화: 10 → 50 (3-5배 빠름, WebSocket에서 안전)
        ...
        def process_full_4h_batch(batch_data):
            ...
            for idx, symbol_data in enumerate(batch_symbols):
                try:
                    ...
                    # 🚀 최적화: to_numpy() 사용한 빠른 DataFrame 변환
                    ...
                    # 🛡️ Rate Limit 방지: 최소 딜레이 (WebSocket 안전)
                    time.sleep(0.05)  # 🚀 최적화: 0.33s → 0.05s (6배 빠름, WebSocket에서 안전)
    ...
    def _incremental_4h_filtering(self, candidate_symbols, cache):
        ...
        new_filtered = []
        batch_size = 50  # 🚀 최적화: 20 → 50 (2-3배 빠름, 딜레이로 안전)
    ...
    def _websocket_15m_filtering(self, candidate_symbols):
        ...
        try:
            ...
            print(f"   📡 15m 버퍼 보유: {len(ws_symbols)}count | 미보유: {len(non_ws_symbols)}count")
        ...
        try:
            ...
            if not self._check_rate_limit_before_scan():
                ...
                if websocket_symbols:
                    ...
                    print("❌ No WebSocket data - 1분 대기 후 재Attempt 권장")
                    return []
            ...
            try:
                ...
                # ============================================================
                # 2Stage: 1일봉 필터링 (고가 대비 시가 50% 이하만 통과)
                # ============================================================
                ...
                def calculate_9am_change(symbol_data):
                    ...
                    try:
                        ...
                        # 🚀 최적화: to_numpy() 사용한 빠른 변환
                        ohlcv = []
                        if ohlcv_df is not None and len(ohlcv_df) > 0:
                            # 빠른 변환: DataFrame을 numpy로
                            timestamps = (ohlcv_df['timestamp'].astype('int64') // 10**6).to_numpy()
                            opens = ohlcv_df['open'].to_numpy()
                            highs = ohlcv_df['high'].to_numpy()
                            lows = ohlcv_df['low'].to_numpy()
                ...
                from concurrent.futures import ThreadPoolExecutor, as_completed
                max_workers = 20  # IP ban 방지를 위한 안전한 병렬화
                ...
                if ("418" in str(e) or "429" in str(e) or 
                    "too many requests" in error_str or "rate limit" in error_str):
                    print(f"🚨 Rate Limit 감지 - 티커 조times 중단: {e}")
                    self._api_rate_limited = True
                    self._last_rate_limit_check = time.time()
                ...
                for i in range(0, len(usdt_symbols), batch_size):
                    # Rate Limit 재Confirm
                    if hasattr(self, '_api_rate_limited') and self._api_rate_limited:
                        print("🚨 Batch Process 중 Rate Limit 감지 - 중단")
                        break
                    ...
                    try:
                        ...
                        if ("418" in str(e) or "429" in str(e) or 
                            "too many requests" in error_str or "rate limit" in error_str):
                            self.logger.warning(f"🚨 Batch {i//batch_size + 1} Rate Limit 감지 - 중단: {e}")
                            self._api_rate_limited = True
                            self._last_rate_limit_check = time.time()
                            break  # 즉시 중단
                        else:
            ...
            if filtered_symbols:
                ...
                symbol_info = []
                for symbol, change, _, _ in top_symbols:
                    symbol_name = symbol.replace('/USDT:USDT', '').replace('/USDT', '')
                    symbol_info.append(f"{symbol_name}(+{change:.1f}%)")

                print(f"📈 최종 선별: {', '.join(symbol_info)}")

                # 전체 선별된 심볼 반환 (최대 150개)
                result_symbols = [symbol for symbol, _, _, _ in filtered_symbols[:150]]
                return result_symbols
            else:
                print("⚠️ 필터링 조건을 만족하는 심볼 없음")
                return []
            ...
            # Error 시에도 빈 배열 반환
            print(f"⚠️ Error 발생으로 스캔 중단 - Filtering 조건 엄격 적용")
            return []
    ...
    def _get_websocket_filtered_symbols(self):
        ...
        try:
            if not hasattr(self, '_websocket_kline_buffer') or not self._websocket_kline_buffer:
                print("⚠️ WebSocket 버퍼가 비어Present")
                return []

            print(f"📡 WebSocket 버퍼 Symbol: {len(self._websocket_kline_buffer)}count")
    ...
    def manual_dca_recovery(self, symbol: str = None):
        ...
        if not hasattr(self, 'dca_recovery') or not self.dca_recovery:
            print("[DCARecover] ❌ DCA Recover 시스템이 Initialize되지 않음")
            return
    ...
    def force_trade_sync(self):
        """강제 Trade 내역 Sync"""
        if not hasattr(self, 'trade_history_sync') or not self.trade_history_sync:
            print("[ERROR] Trade 내역 Sync 시스템이 Initialize되지 않음")
            return False
    ...
    def emergency_dca_recovery(self, symbol: str = None):
        ...
        if hasattr(self, 'dca_recovery') and self.dca_recovery:
            ...
            print("[ERROR] DCA Recover 시스템이 Initialize되지 않음")
            return None
    ...
    def force_enhanced_dca_scan(self):
        ...
        if not hasattr(self, 'dca_recovery') or not self.dca_recovery:
            print("[ERROR] 강화된 DCA Recover 시스템이 Initialize되지 않음")
            return None
        ...
        try:
            ...
            if exchange_positions:
                ...
                print(f"  - 누락 주문 감지: {len(result.get('missing_orders_detected', []))}count")
