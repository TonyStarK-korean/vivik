...
조건 2: (30봉이내 MA5-MA20 데드크로스 AND 10봉이내 MA1-MA5 골든크로스) and (ma5<ma20 or ma5-ma20 이격도 2%이내)
...
3. 60봉이내 MA80-MA480 골든크로스 OR (MA80<MA480 and MA80-MA480 이격도 5%이내)
...
class OneMinuteSurgeEntryStrategy:
    ...
    def __init__(self, api_key=None, secret_key=None, sandbox=False):
        ...
        self._ohlcv_cache_ttl = 1200  # 20분 (캐시 효율성 극대화)
    ...
    def check_5m_supertrend_entry_signal(self, symbol, df_5m):
        ...
        # 🚫 임시 비활성화: SuperTrend 진입 조건을 무조건 통과시킴
        return True

        # 아래 코드는 임시로 비활성화됨 (나중에 재활성화 가능)
        try:
            ...
            if strategy_3m_3rd_enabled and df_3m is not None:
                try:
                    ...
                    if supertrend_signal:
                        ...
                        # 2A. 60봉이내 MA5-MA20 데드크로스
                        condition_3m_c2a = False
                        if df_3m_calc is not None and len(df_3m_calc) >= 60:
                            condition_3m_c2a = self._find_dead_cross(df_3m_calc, 'ma5', 'ma20', recent_n=60)

                        conditions_3m_3rd.append(f"[3분봉 3번째-2A] 60봉이내 MA5-MA20 데드크로스: {condition_3m_c2a}")
            ...
            if strategy_5m_4th_enabled and df_5m is not None and len(df_5m) >= 30:  # 30봉 필요 (약 2.5시간)
                try:
                    ...
                    if df_5m_calc is not None and len(df_5m_calc) >= 30:  # 30봉 필요
                        # ⚡ 조건 1: 15분봉 MA80<MA480 (조기 종료 1단계)
                        condition_5m_d1 = False
                        if df_15m is not None and len(df_15m) >= 20:  # 80→20으로 더 완화 (약 5시간)
                        ...
                        # ✅ 모든 조건 독립적으로 체크 (진입임박 판단을 위해 조기종료 제거)
                        # 초기화: 모든 조건 False로 시작
                        ...
                        # 조건 2: 5분봉 SuperTrend(10-3) 진입 시그널 (항상 체크)
                        supertrend_signal = self.check_5m_supertrend_entry_signal(symbol, df_5m_calc)
                        condition_5m_d2 = supertrend_signal

                        # 조건 3: 30봉이내 MA80-MA480 골든크로스 OR (MA80<MA480 and MA80-MA480 이격도 5%이내)
                        golden_cross_met = False
                        gap_condition_met = False

                        if len(df_5m_calc) >= 30:
                            # 골든크로스 확인
                            golden_cross_met = find_golden_cross_vectorized(df_5m_calc, 'ma80', 'ma480', recent_n=30)

                            # 현재 MA80 < MA480 and 이격도 5% 이내 확인
                        ...
                        if len(df_5m_calc) >= 60:  # 100→60으로 완화
                        ...
                        # 조건 5: 10봉이내 MA5-MA20 골든크로스 (항상 체크)
                        if len(df_5m_calc) >= 10:
                            condition_5m_d5 = find_golden_cross_vectorized(df_5m_calc, 'ma5', 'ma20', recent_n=10)
                        
                        # 상세 로그 제거 (성능 최적화)
                        # conditions_5m_4th.append(f"[5분봉 D전략-5] 10봉이내 MA5-MA20 골든크로스: {condition_5m_d5}")

                        # 전략D 최종 평가: 5개 조건 모두 충족 (원래대로 롤백)
    ...
    def analyze_symbol(self, symbol, cached_ticker=None):
        ...
        try:
            ...
            df_3m, source_3m = safe_fetch_websocket_with_history('3m', 500)
            ...
            df_15m, source_15m = safe_fetch_websocket_with_history('15m', 700)
            ...
            if strategy_3m_3rd_met:  # 전략 통과 (1 AND (2 OR 3) 구조)
                ...
                if is_data_insufficient_c:  # 데이터 부족인 경우
                    ...
                    status_3m_3rd = 'near_entry'
                elif failed_count_preview == 2:
                    status_3m_3rd = 'potential_entry'
                elif failed_count_preview <= 10:  # 3~10개 미충족 → 관심목록 (대폭 완화)
                    status_3m_3rd = 'watchlist'
                else:
                    status_3m_3rd = 'no_signal'
            ...
            if strategy_5m_4th_met_safe:  # 전략 통과 (5개 조건 모두 충족)
                ...
                if failed_5m_4th == 0:  # 🚨 FIX: 모든 조건 통과
                    status_5m_4th = 'entry_signal'
                elif failed_5m_4th == 1:  # 1개 부족
                    status_5m_4th = 'near_entry'
                elif failed_5m_4th == 2:  # 2개 부족
                    status_5m_4th = 'potential_entry'
                elif failed_5m_4th <= 10:  # 3~10개 부족 (대폭 완화)
    ...
    def scan_symbols(self, symbols):
        ...
        if hasattr(self, '_speed_test_mode') and self._speed_test_mode:
            ...
            max_workers = min(len(symbols), 50)  # 최대 50개 동시 처리 (극한 속도)
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future, symbol in submitted_futures:
                    completed_count += 1
                    # 진행 상황 출력 빈도 줄임 (150개 → 250개마다)
                    if completed_count % 250 == 0 or completed_count == len(submitted_futures):
                        print(f"⚡ 진행 중: {completed_count}/{len(submitted_futures)}", end='\r')

                    try:
                        # 🚀 타임아웃 단축: 캐싱으로 대부분 즉시 반환
                        result = future.result(timeout=5)  # 5초 타임아웃 (캐시 히트시 즉시)
                ...
                for symbol, result in symbol_results:
                    ...
                    if False:  # 기존 1분봉-15분봉 조합전략 코드 비활성화
                        ...
                        if failed_count == 0:
                            ...
                            status_msg = "⚠️ 1개 조건 미충족"
                        else:
                            status_msg = f"⚠️ {failed_count}개 조건 미충족"

                        # 통과한 조건 개수 계산 (음수 방지)
                        passed_count = max(0, total_conditions - failed_count)
                        
                        # 🎨 1개 실패(near_entry)인 경우 심볼명을 노란색으로 표시
                        if failed_count == 1:
                            print(f"   \033[93m\033[1m{clean_symbol}\033[0m [+{change_pct:.1f}%] ({passed_count}/{total_conditions}) {status_msg}")
                        else:
                            print(f"   {clean_symbol} [+{change_pct:.1f}%] ({passed_count}/{total_conditions}) {status_msg}")
                        ...
                        for failed_condition in failed_main_conditions:
                            ...
                            if failed_count == 1:
                                print(f"\033[33m      ❌ {failed_condition}\033[0m")
                            else:
                                print(f"\033[91m      ❌ {failed_condition}\033[0m")
        ...
        if potential_entry:
            ...
            for strategy_type in strategy_order:
                ...
                for i in range(0, len(symbol_infos), 5):
                    line_symbols = symbol_infos[i:i+5]
                ...
                try:
                    ...
                    if dca_exit_result and dca_exit_result.get('success'):
                        ...
                        if emergency_exit_result:
                            print(f"[긴급청산] ✅ {clean_symbol} 긴급 청산 완료")
                            return True
                        else:
                            print(f"[긴급청산] ❌ {clean_symbol} 긴급 청산 실패")
                            return False
    ...
    def monitor_positions_realtime(self):
        ...
        # 🔄 순환매 기회 모니터링 및 실행 (30초마다)
        if not hasattr(self, '_last_cyclic_check_time'):
            self._last_cyclic_check_time = 0

        if current_time - self._last_cyclic_check_time > 30:  # 30초
            try:
    ...
    def _full_4h_filtering(self, candidate_symbols):
        """전체 4시간 필터링 - 4봉 이내 시가대비고가 2% 이상 급등 + 4봉이전~0봉 모두 양봉"""
        from concurrent.futures import ThreadPoolExecutor, as_completed
        import time
            ...
            for idx, symbol_data in enumerate(batch_symbols):
                try:
                    ...
                    for i in range(-4, 0):
                        ...
                        if close_price <= open_price:  # 음봉 또는 동일가
                            all_positive = False
                            break
    ...
    def get_filtered_symbols(self, min_change_pct=8.0):  # 4% → 8%로 강화
        ...
        try:
            ...
            # WebSocket 데이터 우선 사용하되, 충분하지 않으면 REST API 폴백
            websocket_symbols = self._get_websocket_filtered_symbols()
            if websocket_symbols and len(websocket_symbols) >= 10:  # 최소 10개 이상일 때만 WebSocket 사용
            ...
            # 1단계: 티커 데이터 수집 (Rate Limit 대응 강화)
            candidate_symbols = []
            
            # 🚨 Rate Limit 체크: 418 에러 감지시 스캔 중단
            if hasattr(self, '_api_rate_limited') and self._api_rate_limited:
                print("🚨 Rate Limit 상태 - 티커 조회 건너뛰기")
                return []
            ...
            try:
                ...
                # 24시간 변동률 기준 상위 500개 선택
                temp_candidates.sort(key=lambda x: x[1], reverse=True)
                top_500_symbols = temp_candidates[:500]
                print(f"📊 1단계 사전 필터링: 24시간 변동률 기준 상위 {len(top_500_symbols)}개 선택")

                # 2단계: 상위 500개에 대해 오늘 09:00 이후 변동률 계산
                ...
                # 상위 500개에 대해 09:00 이후 변동률 재계산
                for idx, (symbol, _, volume_24h, ticker) in enumerate(top_500_symbols):
                    ...
                    # 진행 상황 표시 (100개마다)
                    if (idx + 1) % 100 == 0:
                        print(f"   ⏳ 09:00 이후 변동률 계산 중... {idx + 1}/{len(top_500_symbols)}")
                ...
                print(f"   ✅ 09:00 이후 > 0% 필터링 완료: {len(candidate_symbols)}개")
            ...
            for buffer_key, kline_data in self._websocket_kline_buffer.items():
                if '_1m' in buffer_key and len(kline_data) >= 3:  # 1분봉 데이터 조건 더욱 완화 (10개→3개)
                    ...
                    if len(kline_data) >= 1440:
                        current_price = kline_data[-1][4]  # 최신 종가
                        day_ago_price = kline_data[-1440][4]  # 24시간 전 종가
                        change_pct = ((current_price - day_ago_price) / day_ago_price) * 100
                    else:
                        # 데이터가 부족하면 가용한 모든 데이터로 변동률 추정
                        current_price = kline_data[-1][4]
                        old_price = kline_data[0][4] if len(kline_data) > 0 else current_price
                        if old_price > 0:
                    ...
                    volume_24h = sum(candle[5] for candle in kline_data[-available_candles:])  # 가용한 캔들의 거래량 합계
...
def main():
    ...
    try:
        ...
        while True:
            ...
            # 현재 계좌 포지션 상황 보고 (테이블 형태)
            if strategy.active_positions:
                print(f"📊 [실시간데이터] 활성 포지션 1초 고속 모니터링...")
                print("="*120)
                ...
                for symbol, pos_info in strategy.active_positions.items():
                    clean_symbol = symbol.replace('/USDT:USDT', '').replace('/USDT', '')
                    entry_amount = pos_info.get('entry_amount', 0)
                    leverage = pos_info.get('leverage', strategy.leverage)
                    ...
                    # 현재가 조회 및 수익률 계산
                    try:
                        current_price = strategy.get_current_price(symbol)
                        entry_price = pos_info.get('entry_price', 0)
                        
                        # DCA 시스템의 평균가 우선 사용 (동기화 개선)
                        if strategy.dca_manager and symbol in strategy.dca_manager.positions:
                            dca_position = strategy.dca_manager.positions[symbol]
                            if dca_position.is_active:
                                entry_price = dca_position.average_price
                                # DCA 시스템의 실제 투자금액 사용
                                entry_amount = dca_position.total_amount_usdt if hasattr(dca_position, 'total_amount_usdt') else entry_amount
                ...
                total_original_profit_pct = total_leverage_profit_pct / strategy.leverage  # 원금 수익률
