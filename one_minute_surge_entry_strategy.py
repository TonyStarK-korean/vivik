# -*- coding: utf-8 -*-
"""
전략A + 전략B + 전략C 조합 시스템 (OR 조합)
SuperClaude Expert Mode Implementation + Multi-Timeframe Strategy System

🔥 필터링 조건: 일봉상 30봉이내 시가대비고가 30%이상 상승 (시작시 1회 스캔)

- 전략A: 3분봉 시세 초입 포착 (6개 조건 AND 5분봉 SuperTrend) - ✅ 활성화됨
- 전략B: 5분봉 초입 초강력 타점 (3개 조건 모두 충족 필요) - ✅ 활성화됨
- 전략C: 15분봉 대박파동 맥점 (3개 복합 조건 모두 충족) - ✅ 신규 추가
...
🔥 필터링 조건 (시작시 1회 스캔):
------------------------------------------------------------------------
- 일봉상 30봉이내 시가대비고가 30%이상 상승

전략 A: 3분봉 시세 초입 포착 (복합 논리 조건 + SuperTrend 모두 충족 필요) - ✅ 활성화됨:
------------------------------------------------------------------------
...
전략 B: 5분봉 초입 초강력 타점 (5개 조건 모두 충족 필요) - ✅ 활성화됨:
----------------------------------------------------------------------
1. 15분봉 MA80<MA480
...
전략 C: 15분봉 대박파동 맥점 (3개 복합 조건 모두 충족 필요) - ✅ 신규 추가:
----------------------------------------------------------------------
1. (MA80<MA480 OR BB 복합조건) AND 5봉내 BB80-BB200 골든크로스 AND 100봉내 BB200-MA480 교차패턴
   BB 복합조건: (BB80<BB200 AND 이격도 5%이내) OR 100봉내 BB480(표편1.5)-BB200(표편2) 골든크로스
2. 5봉이내 MA5<MA20 AND 5봉이내 시가<MA5 AND 종가>MA5
3. MA5<MA20 OR 3봉이내 MA5-MA20 골든크로스
...
# 🛡️ Binance Rate Limiter import
try:
    from binance_rate_limiter import RateLimitedExchange, BinanceRateLimiter
    HAS_RATE_LIMITER = True
except ImportError:
    print("[INFO] binance_rate_limiter.py 없음 - 기본 Rate Limiting 사용")
    RateLimitedExchange = None
    BinanceRateLimiter = None
...
# 🚀 Indicators module import (리팩토링: 지표 계산 함수 분리)
try:
    ...
    HAS_INDICATORS_MODULE = True
except ImportError:
    print("[WARNING] indicators.py 없음 - 기존 메서드 사용")
    HAS_INDICATORS_MODULE = False

# 🚀 WebSocket Hybrid System import (150개 심볼 실시간 관리)
try:
    ...
    print("[INFO] WebSocket Hybrid System 없음 - REST API 모드 사용")
    BulkWebSocketKlineManager = None
    WebSocketDefenseSystem = None
    HAS_BULK_WS = False

# 🚀 Cache Manager import (리팩토링: 캐시 관리 모듈 분리)
try:
    from cache_manager import CacheManager
    HAS_CACHE_MANAGER = True
except ImportError:
    print("[WARNING] cache_manager.py 없음 - 기존 캐시 메서드 사용")
    CacheManager = None
...
# 전략 조건 상세 설명 (indicators.py에서 import됨)
# ✅ 리팩토링: indicators 모듈에서 STRATEGY_CONDITION_DETAILS 가져옴
# 아래 정의는 indicators.py가 없을 경우 폴백용
if not HAS_INDICATORS_MODULE:
    STRATEGY_CONDITION_DETAILS = {
        # 전략 C 조건들
        'C1': {
            'name': '조건1',
...
class OneMinuteSurgeEntryStrategy:
    ...
    def __init__(self, api_key=None, secret_key=None, sandbox=False):
        ...
        # 🛡️ IP 밴 방지를 위한 Exchange Wrapper 설정
        self._ip_ban_detected = False
        ...
        while retry_count < max_retries:
            try:
                ...
                # 🛡️ Rate Limiter 적용
                if HAS_RATE_LIMITER:
                    self.exchange = RateLimitedExchange(base_exchange, self.logger)
                    self.rate_limiter = self.exchange.rate_limiter
                    self.logger.info("🛡️ Rate Limiter 활성화됨")
                else:
                    self.exchange = base_exchange
                    self.rate_limiter = None
                    self.logger.warning("⚠️ Rate Limiter 비활성화 - IP 밴 위험 존재")
                ...
                if ("418" in error_str or "429" in error_str or "banned" in error_str.lower() or 
                    ...
                    self.logger.warning(f"🚨 API Rate Limit/IP 밴 감지: {error_str}")
                    
                    # Rate limiter가 있으면 에러 기록
                    if hasattr(self, 'rate_limiter') and self.rate_limiter:
                        status_code = 418 if "418" in error_str else 429
                    ...
                    print("🔄 WebSocket 전용 모드로 전환 (REST API 차단)")
                    
                    # 최소한의 거래소 설정
                    try:
                        # API 키 없이 공개 모드로 설정 (마켓 정보만)
                        public_exchange = ccxt.binance({
                            'sandbox': sandbox,
                            'enableRateLimit': False,  # Rate limit 비활성화
                            'timeout': 10000,
                            'options': {'defaultType': 'future'}
                        })
                        
                        if HAS_RATE_LIMITER:
                            self.exchange = RateLimitedExchange(public_exchange, self.logger)
                            self.rate_limiter = self.exchange.rate_limiter
                        else:
                            self.exchange = public_exchange
                            self.rate_limiter = None
                        
                        self.logger.info("⚠️ WebSocket 전용 모드 활성화")
                        break
                    except:
                        pass
                else:
        ...
        # 🛡️ Rate Limiter 상태 초기화
        self._api_rate_limited = getattr(self, '_api_rate_limited', False)
        
        # Rate limiter 상태 주기적 출력
        if self.rate_limiter:
            self._last_rate_limiter_status_time = 0
        ...
        # 💰 시드 설정 (전체 수익률 계산용)
        self.initial_seed = 1000.0  # 초기 시드 $1000 (실제 잔고에 맞춤)
        ...
        # 주기적 포지션 동기화 시작 (API 키가 있을 때만)
        if api_key and secret_key:
            self.start_periodic_position_sync()
        ...
        # 🚨 긴급 WebSocket 전용 모드 초기화
        self.emergency_mode = None
        try:
            from emergency_websocket_mode import EmergencyWebSocketMode
            self.emergency_mode = EmergencyWebSocketMode(self)
            self.emergency_mode.override_api_calls()
            print("🚨 긴급 WebSocket 전용 모드 강제 활성화 (Rate Limit 대응)")
        except Exception as e:
            print(f"⚠️ 긴급 모드 초기화 실패: {e}")
        ...
        # 🚀 Bulk WebSocket Hybrid System 초기화 (150개 심볼 관리)
        self.bulk_ws_manager = None
        self.ws_defense_system = None
        ...
        if HAS_BULK_WS and self.ws_kline_manager:
            try:
                ...
                self.logger.info("🚀 Bulk WebSocket Hybrid System 초기화 완료")
                print("🚀 WebSocket Hybrid System: 150개 심볼 실시간 관리 활성화")

            except Exception as e:
                self.logger.error(f"❌ Bulk WebSocket System 초기화 실패: {e}")
                print(f"[Bulk WebSocket] ❌ 초기화 실패: {e}")
                self.bulk_ws_manager = None
                self.ws_defense_system = None
        else:
            if not HAS_BULK_WS:
                self.logger.info("ℹ️ Bulk WebSocket System 비활성화 - bulk_websocket_kline_manager.py 필요")
            elif not self.ws_kline_manager:
                self.logger.info("ℹ️ Bulk WebSocket System 비활성화 - 기본 WebSocket 관리자 필요")
        ...
        # 🚀 Cache Manager 초기화 (리팩토링: 통합 캐시 시스템)
    ...
    def _get_cached_data(self, cache_key):
        """
        캐시된 데이터 조회 (리팩토링: cache_manager 사용)
        """
        if HAS_CACHE_MANAGER and self.cache_manager:
    ...
    def _set_cached_data(self, cache_key, data):
        """
        데이터 캐시에 저장 (리팩토링: cache_manager 사용)
        """
        if HAS_CACHE_MANAGER and self.cache_manager:
    ...
    def format_condition_result(self, condition_code: str, result: bool, extra_info: str = "") -> str:
        """
        조건 체크 결과를 상세하게 포맷팅 (리팩토링: indicators 모듈 사용)
        """
        if HAS_INDICATORS_MODULE:
            # 🚀 리팩토링된 모듈 함수 사용
            return format_condition_result(condition_code, result, extra_info)
        else:
            # 폴백: 기존 로직 유지
    ...
    def _get_cached_markets(self):
        """
        캐시된 마켓 정보 조회 (리팩토링: cache_manager 사용)
        초기 스캔 시간 90% 단축: 1시간 캐싱
        """
        if HAS_CACHE_MANAGER and self.cache_manager:
            return self.cache_manager.get_cached_markets(self.exchange)
        else:
            # 폴백: 기존 로직
            try:
    ...
    def _smart_hybrid_data_fetch(self, symbol, clean_symbol):
        ...
        try:
            ...
            if ws_sufficient:
                ...
                # 🚀 IP 밴 방지: WebSocket 데이터 부족시 완전 스킵 (REST API 호출 안함)
                self.logger.debug(f"⚠️ [{clean_symbol}] WebSocket 전용 모드 - REST API 완전 차단")
                return None
    ...
    def calculate_indicators(self, df):
        """
        기술적 지표 계산 (리팩토링: indicators 모듈 사용)

        ✅ 개선사항:
        - indicators.py 모듈로 분리하여 재사용성 향상
        - 로거 통합으로 일관된 로깅 제공
        """
        if HAS_INDICATORS_MODULE:
            try:
                # 🚀 리팩토링된 모듈 함수 사용
                return calculate_indicators(df, logger=self.logger)
            except Exception as e:
                # indicators.py 모듈이 있지만 함수 호출 실패시 폴백
                self.logger.warning(f"[FALLBACK] indicators.py 함수 호출 실패: {e} - 기존 메서드 사용")
                return self._calculate_indicators_legacy(df)
        else:
            # 폴백: 기존 로직 유지 (indicators.py 없을 경우)
            self.logger.warning("[FALLBACK] indicators.py 없음 - 기존 메서드 사용")
            # 기존 구현 유지 (안전성)
            return self._calculate_indicators_legacy(df)
    
    def _calculate_indicators_legacy(self, df):
        """기본 기술적 지표 계산 - indicators.py 없을 때 사용하는 폴백 메서드"""
        try:
            ...
            if len(df) >= 80:
                ...
                df['bb80_upper'] = df['close']
                df['bb80_lower'] = df['close'] 
                df['bb80_middle'] = df['close']
            ...
            if len(df) >= 200:
                ...
                df['bb200_upper'] = df['close']
                df['bb200_lower'] = df['close']
                df['bb200_middle'] = df['close']
            ...
            if len(df) >= 480:
                ...
                df['bb480_upper'] = df['close']
                df['bb480_lower'] = df['close']
                df['bb480_middle'] = df['close']
            
            # BB600 (기간 600, 표준편차 2.0)
            if len(df) >= 600:
                bb600_ma = df['close'].rolling(window=600).mean()
                bb600_std = df['close'].rolling(window=600).std()
                df['bb600_upper'] = bb600_ma + (bb600_std * 2.0)
                df['bb600_lower'] = bb600_ma - (bb600_std * 2.0)
                df['bb600_middle'] = bb600_ma
            else:
                df['bb600_upper'] = df['close']
                df['bb600_lower'] = df['close']
                df['bb600_middle'] = df['close']
            ...
            self.logger.error(f"기본 지표 계산 실패: {e}")
            return None

    def calculate_supertrend(self, df, period=10, multiplier=3.0):
        """
        SuperTrend 지표 계산 (리팩토링: indicators 모듈 사용)
        """
        if HAS_INDICATORS_MODULE:
            try:
                # 🚀 리팩토링된 모듈 함수 사용
                return calculate_supertrend(df, period=period, multiplier=multiplier, logger=self.logger)
            except Exception as e:
                # indicators.py 모듈이 있지만 함수 호출 실패시 폴백
                self.logger.warning(f"[FALLBACK] indicators.py SuperTrend 함수 호출 실패: {e} - 기존 메서드 사용")
                return self._calculate_supertrend_legacy(df, period, multiplier)
        else:
            # 폴백: 기존 로직 유지
            return self._calculate_supertrend_legacy(df, period, multiplier)
    
    def _calculate_supertrend_legacy(self, df, period=10, multiplier=3.0):
        """SuperTrend 지표 계산 - indicators.py 없을 때 사용하는 폴백 메서드"""
        try:
            ...
            for i in range(len(df)):
                if i == 0:
                    supertrend.append(df['upper_band'].iloc[i])
                    direction.append(1)
                    continue
    ...
    def check_5m_supertrend_entry_signal(self, symbol, df_5m):
        ...
        try:
            ...
            if 'supertrend_direction' in df_5m_calc.columns:
                ...
                # 컬럼이 없으면 SuperTrend 조건 실패
                self.logger.warning(f"SuperTrend 컬럼 없음 - 조건 실패: {symbol}")
                return False
            ...
            # 조건 3 제거: 너무 완화된 조건으로 인한 잘못된 진입 방지
            # (기존에는 현재가 > SuperTrend 값이면 무조건 진입했으나, 이는 정확한 시그널이 아님)
    ...
    def check_strategy_c_conditions(self, symbol, df_15m, df_3m=None, df_5m=None):
        """
        전략C: 15분봉 대박파동 맥점 조건 체크
        ...
        try:
            ...
            # 15분봉 데이터 검증
            if df_15m is None or len(df_15m) < 50:  # 50봉 이상 필요
                conditions.append("[전략C] 15분봉 데이터 부족 (최소 50봉 필요)")
                return False, conditions
            
            # 15분봉 지표 계산
            df_15m_calc = self.calculate_indicators(df_15m)
            if df_15m_calc is None:
                conditions.append("[전략C] 15분봉 지표 계산 실패")
                return False, conditions
            
            # === 조건 1: MA80<MA480 AND ((BB80상단선<BB200상단선 AND BB80-BB200 이격도 5%이내) OR 100봉이내 BB80상단선-BB200상단선 골든크로스) ===
            latest_row = df_15m_calc.iloc[-1]
            
            # MA80<MA480 체크
            ma80_below_ma480 = False
            if (pd.notna(latest_row['ma80']) and pd.notna(latest_row['ma480']) and
                latest_row['ma80'] < latest_row['ma480']):
                ma80_below_ma480 = True
            
            # BB80, BB200 상단선 계산 (없으면 계산)
            if 'bb80_upper' not in df_15m_calc.columns:
                df_15m_calc['bb80_middle'] = df_15m_calc['close'].rolling(window=80).mean()
                df_15m_calc['bb80_std'] = df_15m_calc['close'].rolling(window=80).std()
                df_15m_calc['bb80_upper'] = df_15m_calc['bb80_middle'] + (df_15m_calc['bb80_std'] * 2.0)
            
            if 'bb200_upper' not in df_15m_calc.columns:
                df_15m_calc['bb200_middle'] = df_15m_calc['close'].rolling(window=200).mean()
                df_15m_calc['bb200_std'] = df_15m_calc['close'].rolling(window=200).std()
                df_15m_calc['bb200_upper'] = df_15m_calc['bb200_middle'] + (df_15m_calc['bb200_std'] * 2.0)
            
            # BB80상단선<BB200상단선 AND 이격도 5%이내
            bb_condition_1a = False
            if (pd.notna(latest_row['bb80_upper']) and pd.notna(latest_row['bb200_upper']) and
                latest_row['bb80_upper'] < latest_row['bb200_upper'] and latest_row['bb200_upper'] > 0):
                gap_pct = ((latest_row['bb200_upper'] - latest_row['bb80_upper']) / latest_row['bb200_upper']) * 100
                if gap_pct <= 5.0:
                    bb_condition_1a = True
            
            # OR 100봉이내 BB480상단선(표준편차1.5)-BB200상단선(표준편차2) 골든크로스
            bb480_bb200_golden_cross = False
            
            # BB480 상단선 계산 (표준편차 1.5)
            if 'bb480_upper_std15' not in df_15m_calc.columns:
                df_15m_calc['bb480_middle'] = df_15m_calc['close'].rolling(window=480).mean()
                df_15m_calc['bb480_std'] = df_15m_calc['close'].rolling(window=480).std()
                df_15m_calc['bb480_upper_std15'] = df_15m_calc['bb480_middle'] + (df_15m_calc['bb480_std'] * 1.5)
            
            if len(df_15m_calc) >= 100:
                recent_100 = df_15m_calc.tail(100)
                for i in range(1, len(recent_100)):
                    prev_row = recent_100.iloc[i-1]
                    curr_row = recent_100.iloc[i]
                    
                    # 데이터 유효성 체크
                    if (pd.notna(prev_row['bb480_upper_std15']) and pd.notna(prev_row['bb200_upper']) and
                        pd.notna(curr_row['bb480_upper_std15']) and pd.notna(curr_row['bb200_upper'])):
                        
                        # 이전봉 BB480상단선(표준편차1.5) <= BB200상단선(표준편차2) AND 현재봉 BB480상단선 > BB200상단선
                        golden_cross_pattern = (prev_row['bb480_upper_std15'] <= prev_row['bb200_upper'] and 
                                               curr_row['bb480_upper_std15'] > curr_row['bb200_upper'])
                        
                        if golden_cross_pattern:
                            bb480_bb200_golden_cross = True
                            break
            
            bb_condition_1 = bb_condition_1a or bb480_bb200_golden_cross
            
            # OR 5봉이내 BB80상단선-BB200상단선 골든크로스
            bb_golden_cross = False
            if len(df_15m_calc) >= 5:
                bb_golden_cross = self._find_golden_cross(df_15m_calc, 'bb80_upper', 'bb200_upper', recent_n=5)
            
            # AND 100봉이내 BB200-MA480 OR BB200-BB480(표편1.5) 교차 패턴
            bb200_ma480_cross = False
            bb200_bb480_cross = False
            ...
            if len(df_15m_calc) >= 100:
                ...
                for i in range(1, len(recent_100)):
                    ...
                    # 패턴 1: BB200-MA480 교차 체크
                    if (pd.notna(prev_row['bb200_upper']) and pd.notna(prev_row['ma480']) and
                        pd.notna(curr_row['bb200_upper']) and pd.notna(curr_row['ma480'])):
                        
                        # BB200상단선과 MA480 교차 (골든크로스 또는 데드크로스)
                        ma480_golden_cross = (prev_row['bb200_upper'] <= prev_row['ma480'] and 
                                            curr_row['bb200_upper'] > curr_row['ma480'])
                        ma480_dead_cross = (prev_row['bb200_upper'] >= prev_row['ma480'] and 
                                          curr_row['bb200_upper'] < curr_row['ma480'])
                        
                        if ma480_golden_cross or ma480_dead_cross:
                            bb200_ma480_cross = True
                    
                    # 패턴 2: BB200-BB480(표편1.5) 교차 체크
                    if (pd.notna(prev_row['bb200_upper']) and pd.notna(prev_row['bb480_upper_std15']) and
                        pd.notna(curr_row['bb200_upper']) and pd.notna(curr_row['bb480_upper_std15'])):
                        
                        # BB200상단선과 BB480상단선(표편1.5) 교차
                        bb480_golden_cross = (prev_row['bb200_upper'] <= prev_row['bb480_upper_std15'] and 
                                            curr_row['bb200_upper'] > curr_row['bb480_upper_std15'])
                        bb480_dead_cross = (prev_row['bb200_upper'] >= prev_row['bb480_upper_std15'] and 
                                          curr_row['bb200_upper'] < curr_row['bb480_upper_std15'])
                        
                        if bb480_golden_cross or bb480_dead_cross:
                            bb200_bb480_cross = True
                    
                    # 둘 중 하나라도 발견되면 조건 충족
                    if bb200_ma480_cross or bb200_bb480_cross:
                        break
            
            # 조건1D: BB200-MA480 교차 OR BB200-BB480(표편1.5) 교차
            condition_1d_cross = bb200_ma480_cross or bb200_bb480_cross
            
            # 최종 조건 1: (1A OR 1B) AND 1C AND 1D
            bb_condition = bb_condition_1 or bb_golden_cross  # 1B OR 1C
            condition_1_met = (ma80_below_ma480 or bb_condition_1) and bb_golden_cross and condition_1d_cross
            
            conditions.append(f"[전략C-1A] MA80<MA480: {ma80_below_ma480}")
            conditions.append(f"[전략C-1B1] BB80<BB200 AND 이격도≤5%: {bb_condition_1a}")
            conditions.append(f"[전략C-1B2] 100봉내 BB480-BB200 골든크로스: {bb480_bb200_golden_cross}")
            conditions.append(f"[전략C-1B] 조건1B (1B1 OR 1B2): {bb_condition_1}")
            conditions.append(f"[전략C-1C] 5봉내 BB80-BB200 골든크로스: {bb_golden_cross}")
            conditions.append(f"[전략C-1D1] 100봉내 BB200-MA480 교차: {bb200_ma480_cross}")
            conditions.append(f"[전략C-1D2] 100봉내 BB200-BB480(표편1.5) 교차: {bb200_bb480_cross}")
            conditions.append(f"[전략C-1D] 조건1D (1D1 OR 1D2): {condition_1d_cross}")
            conditions.append(f"[전략C-1] 조건1 ((1A OR 1B) AND 1C AND 1D): {condition_1_met}")
            
            # === 조건 2: 5봉이내 MA5<MA20 AND 5봉이내 시가<MA5 AND 종가>MA5 ===
            if len(df_15m_calc) >= 5:
                recent_5 = df_15m_calc.tail(5)
                
                # 5봉이내 MA5<MA20 체크
                ma5_below_ma20_found = False
                open_below_ma5_found = False
                
                for _, row in recent_5.iterrows():
                    if (pd.notna(row['ma5']) and pd.notna(row['ma20']) and
                        row['ma5'] < row['ma20']):
                        ma5_below_ma20_found = True
                    
                    if (pd.notna(row['open']) and pd.notna(row['ma5']) and
                        row['open'] < row['ma5']):
                        open_below_ma5_found = True
                
                # 현재 종가>MA5 체크
                close_above_ma5 = False
                if (pd.notna(latest_row['close']) and pd.notna(latest_row['ma5']) and
                    latest_row['close'] > latest_row['ma5']):
                    close_above_ma5 = True
                
                condition_2_met = ma5_below_ma20_found and open_below_ma5_found and close_above_ma5
                
                conditions.append(f"[전략C-2A] 5봉내 MA5<MA20: {ma5_below_ma20_found}")
                conditions.append(f"[전략C-2B] 5봉내 시가<MA5: {open_below_ma5_found}")
                conditions.append(f"[전략C-2C] 종가>MA5: {close_above_ma5}")
                conditions.append(f"[전략C-2] 조건2 (2A AND 2B AND 2C): {condition_2_met}")
            
            # === 조건 3: MA5<MA20 OR 3봉이내 MA5-MA20 골든크로스 ===
            # 현재 MA5<MA20
            current_ma5_below_ma20 = False
            if (pd.notna(latest_row['ma5']) and pd.notna(latest_row['ma20']) and
                latest_row['ma5'] < latest_row['ma20']):
                current_ma5_below_ma20 = True
            
            # 3봉이내 MA5-MA20 골든크로스
            ma5_ma20_golden_cross = False
            if len(df_15m_calc) >= 3:
                ma5_ma20_golden_cross = self._find_golden_cross(df_15m_calc, 'ma5', 'ma20', recent_n=3)
            
            condition_3_met = current_ma5_below_ma20 or ma5_ma20_golden_cross
            
            conditions.append(f"[전략C-3A] 현재 MA5<MA20: {current_ma5_below_ma20}")
            conditions.append(f"[전략C-3B] 3봉내 MA5-MA20 골든크로스: {ma5_ma20_golden_cross}")
            conditions.append(f"[전략C-3] 조건3 (3A OR 3B): {condition_3_met}")
            
            # === 최종 결과: 모든 조건 AND ===
            final_result = condition_1_met and condition_2_met and condition_3_met
            conditions.append(f"[전략C-최종] 전략C (조건1 AND 조건2 AND 조건3): {final_result}")
            
            # 디버깅 로그
            if final_result:
                self.logger.info(f"✅ {clean_symbol} 전략C: 15분봉 대박파동 맥점 조건 충족!")
            else:
                missing = []
                ...
                if not condition_2_met:
                    missing.append("조건2")
                if not condition_3_met:
                    missing.append("조건3")
                self._write_debug_log(f"[DEBUG-전략C] {clean_symbol}: 미충족 조건 - {', '.join(missing)}")
            
            return final_result, conditions
            
        except Exception as e:
            self.logger.error(f"{clean_symbol} 전략C 조건 체크 실패: {e}")
            return False, [f"[전략C] 오류: {e}"]

    def _check_daily_surge_condition(self, symbol):
        """
        일봉상 30봉이내 시가대비고가 30%이상 상승 조건 체크
        """
        try:
            # 심볼명 정리
            clean_symbol = symbol.replace('/USDT:USDT', '').replace('/USDT', '')
            ...
            # 일봉 데이터 조회 - 원본 API 직접 호출
            try:
                # 안전 래퍼 우회하고 원본 API 직접 사용
                if hasattr(self, '_original_fetch_ohlcv'):
                    raw_data = self._original_fetch_ohlcv(binance_symbol, '1d', limit=35)
                else:
                    # 새 거래소 인스턴스로 직접 조회
                    import ccxt
                    from binance_config import BinanceConfig
                    
                    fresh_exchange = ccxt.binance({
                    ...
                    raw_data = fresh_exchange.fetch_ohlcv(binance_symbol, '1d', limit=35)
                
                if raw_data and len(raw_data) > 0:
                    import pandas as pd
                    df_1d = pd.DataFrame(raw_data, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
                else:
                    df_1d = None
            except Exception as api_err:
                # API 직접 호출 실패시 기존 방식 폴백
                df_1d = self.get_ohlcv_data(symbol, '1d', 35)
            ...
            # 최근 30봉 체크
            recent_30_candles = df_1d.tail(30)
            
            surge_found = False
            max_surge = 0
            surge_details = []
            
            for i, row in recent_30_candles.iterrows():
                open_price = row['open']
                high_price = row['high']
                low_price = row['low']
                ...
                if open_price > 0:
                    ...
                    # 상위 급등들을 기록 (디버깅용)
                    if surge_pct >= 30.0:  # 30% 이상 급등 기록
                        surge_details.append(f"{surge_pct:.1f}%")
                        surge_found = True
                        return True, f"일봉 급등 조건 충족 (30봉내 30%이상): {surge_pct:.1f}%"
            
            
            return False, f"30봉내 일봉 30% 급등 없음 (최대: {max_surge:.1f}%)"
            
        except Exception as e:
            return False, f"일봉 데이터 조회 실패: {e}"
    
    def _initialize_daily_surge_filter(self):
        """시작시 1회만 실행되는 일봉 급등률 필터링"""
        try:
            print("🔍 일봉 급등률 필터링 시작 (30봉내 30% 급등 조건)...")
            
            # 필터링된 심볼 저장용
            self._daily_surge_passed_symbols = set()
            
            # 바이낸스 선물 심볼 목록 가져오기
            if not hasattr(self, '_all_symbols_cache'):
                try:
                    markets = self.exchange.load_markets()
                    self._all_symbols_cache = [
                        symbol for symbol, market in markets.items() 
                        if market.get('future', False) and 'USDT' in symbol
                    ]
                except Exception as e:
                    print(f"⚠️ 심볼 목록 조회 실패: {e}")
                    return
            
            total_symbols = len(self._all_symbols_cache)
            passed_symbols = []
            
            print(f"📊 총 {total_symbols}개 심볼 일봉 급등률 체크 중...")
            ...
            for i in range(0, total_symbols, batch_size):
                batch = self._all_symbols_cache[i:i+batch_size]
                
                for symbol in batch:
                    try:
                        surge_passed, surge_msg = self._check_daily_surge_condition(symbol)
                        if surge_passed:
                            self._daily_surge_passed_symbols.add(symbol)
                            clean_symbol = symbol.replace('/USDT:USDT', '').replace('/USDT', '')
                            passed_symbols.append(clean_symbol)
                        
                        processed += 1
                        if processed % 100 == 0:
                            print(f"🔍 진행률: {processed}/{total_symbols} ({processed/total_symbols*100:.1f}%) - 통과: {len(passed_symbols)}개")
            ...
            print(f"✅ 일봉 급등률 필터링 완료!")
            print(f"📈 총 {total_symbols}개 중 {len(passed_symbols)}개 심볼이 30% 급등 조건 통과")
            
            if passed_symbols:
                print(f"🎯 통과 심볼 샘플 (처음 10개): {', '.join(passed_symbols[:10])}")
            
            return len(passed_symbols)
            
        except Exception as e:
            print(f"❌ 일봉 급등률 필터링 실패: {e}")
            # 실패시 빈 세트로 초기화 (모든 심볼 차단)
            self._daily_surge_passed_symbols = set()
            return 0
    
    def _is_daily_surge_passed(self, symbol):
        """필터링된 심볼 목록에서 체크 (빠른 조회)"""
        if not hasattr(self, '_daily_surge_passed_symbols'):
            # 아직 초기화되지 않았으면 기존 방식 사용
            surge_passed, _ = self._check_daily_surge_condition(symbol)
            return surge_passed
        
        return symbol in self._daily_surge_passed_symbols

    def check_surge_entry_conditions(self, symbol, df_1m, df_3m, df_1d, df_15m=None, df_5m=None, change_24h=0):
        """전략A + 전략B + 전략C OR 조건 체크 (A=기존 3분봉-3번째, B=기존 5분봉-4번째, C=신규 15분봉)"""
        try:
            # 🔒 안전장치: 매개변수 초기화 확인
            if df_15m is None:
            ...
            strategy_3m_additional_enabled = False  # 🚫 비활성화 (전략A, B, C만 사용)
            ...
            # ========== 최적화된 스캔 순서: 전략A(3분봉) → 전략B(5분봉) → 전략C(15분봉) ==========

            # 전략 1, 2 완전 제거됨 - 전략A, B, C만 사용
            ...
            if strategy_3m_2nd_enabled and df_3m is not None and df_1d is not None:
                try:
                    ...
                    # 3. 60봉이내 ma20-bb600(표준편차 2.0)상단선 골든크로스 OR 현재 MA20 > BB600
            ...
            # ========== 전략B: 5분봉 초입 초강력 타점 ==========
            strategy_5m_4th_enabled = True  # 전략B 활성화
            ...
            if strategy_5m_4th_enabled and df_5m is not None and len(df_5m) >= 30:  # 30봉 필요 (약 2.5시간)
                try:
                    ...
                    if df_5m_calc is not None and len(df_5m_calc) >= 30:  # 30봉 필요
                        ...
                        # ⚡ 성능 최적화: 실패율이 높은 조건들을 빠른 순서로 체크 (엄격한 AND 조건)
                        
                        # 조건 1: 15분봉 MA80<MA480 체크
                        ...
                        # 조건 1 통과 시에만 조건 5 체크
                            ...
                            # 조건 1,5 모두 통과 시에만 조건 3 체크
                            elif condition_5m_d5:
                                ...
                                # 조건 1,3,5 모두 통과 시에만 조건 4 체크
                                elif condition_5m_d3:
                                    ma480_downtrend_10 = False
                                    bb200_ma480_golden = False

                                    if len(df_5m_calc) >= 60:
                                        # MA480이 5연속 이상 우하향 확인 (최근 60봉 내에서)
                                        recent_data = df_5m_calc.tail(60)
                                        ...
                                        # 조건 1,3,4,5 모두 통과 시에만 조건 2 체크
                                        elif condition_5m_d4:
                                            ...
                                            if not condition_5m_d2:
                                                ...
                                                conditions_5m_4th.append("ㄴ SuperTrend 진입 신호 없음")
                                            else:
                                                # 모든 조건 통과 - 전략B 성공!
                                                strategy_5m_4th_met = True
                        ...
                        # 전략B 상세 디버그 출력 (모든 경우)
                        self._write_debug_log(f"[DEBUG-전략B] {symbol}: 조건1={condition_5m_d1}, 조건2={condition_5m_d2}, 조건3={condition_5m_d3}, 조건4={condition_5m_d4}, 조건5={condition_5m_d5}, 미충족={missing_conditions_d}개")
            ...
            # ========== 전략C: 15분봉 대박파동 맥점 ==========
            strategy_c_enabled = True  # 전략C 활성화 (신규)
            strategy_c_met = False
            conditions_c = []
            
            if strategy_c_enabled and df_15m is not None:
                try:
                    strategy_c_met, conditions_c = self.check_strategy_c_conditions(symbol, df_15m, df_3m, df_5m)
                except Exception as e:
                    conditions_c = [f"[전략C] 오류 발생: {str(e)}"]
                    strategy_c_met = False
            else:
                if not strategy_c_enabled:
                    conditions_c.append("[전략C] 🚫 비활성화됨")
                else:
                    conditions_c.append("[전략C] 15분봉 데이터 없음")
                    strategy_c_met = False
            
            # 최종 결과: 전략A OR 전략B OR 전략C
            all_conditions_met = strategy_3m_3rd_met or strategy_5m_4th_met or strategy_c_met

            # 조건 목록에 활성화된 전략 조건 추가 (전략A, B, C)
            conditions.extend(conditions_3m_3rd)
            conditions.extend(conditions_5m_4th)
            conditions.extend(conditions_c)
            ...
            if all_conditions_met:
                ...
                if strategy_3m_3rd_met:
                    strategy_names.append("전략A(3분봉 시세초입)")
                    strategy_types.append("전략A: 3분봉 시세 초입 포착")

                if strategy_5m_4th_met:
                    strategy_names.append("전략B(5분봉 초강력타점)")
                    strategy_types.append("전략B: 5분봉 초입 초강력 타점")
                    
                if strategy_c_met:
                    strategy_names.append("전략C(15분봉 대박파동맥점)")
                    strategy_types.append("전략C: 15분봉 대박파동 맥점")

                # 텔레그램 알림용 전략 타입 결정
                if len(strategy_types) == 3:
                    # 세 전략 모두 충족된 경우
                    strategy_type = "전략A+B+C: 3분+5분+15분봉 최강 복합"
                elif len(strategy_types) == 2:
                    # 두 전략 충족된 경우
                    if strategy_3m_3rd_met and strategy_5m_4th_met:
                        strategy_type = "전략A+B: 3분봉+5분봉 복합"
                    elif strategy_3m_3rd_met and strategy_c_met:
                        strategy_type = "전략A+C: 3분봉+15분봉 복합"
                    elif strategy_5m_4th_met and strategy_c_met:
                        strategy_type = "전략B+C: 5분봉+15분봉 복합"
                    else:
                        strategy_type = " 및 ".join(strategy_types)
                elif len(strategy_types) == 1:
                    # 하나만 충족된 경우
                    strategy_type = strategy_types[0]
                else:
                    # 예외 상황 (발생하지 않아야 함)
                    strategy_type = "전략A: 3분봉 시세 초입 포착"
            ...
            # 전략A, B, C는 이제 개별적으로 조건 체크하고 상태 결정
            else:
                ...
                strategy_c_met_debug = locals().get('strategy_c_met', False)
                self._write_debug_log(f"❌ {symbol} 진입 조건 미충족 (전략A: {strategy_3m_3rd_met}, 전략B: {strategy_5m_4th_met_debug}, 전략C: {strategy_c_met_debug})")
            ...
            if "418" in str(e) or "too many requests" in str(e).lower():
                ...
                self.logger.error(f"{symbol} 전략A/B/C 진입 조건 체크 실패: {e}")
    ...
    def analyze_symbol(self, symbol, cached_ticker=None):
        ...
        try:
            ...
            # 🚀 일봉 급등 조건 체크 (사전 필터링된 목록 사용)
            if not self._is_daily_surge_passed(symbol):
                # 일봉 조건 미충족시 즉시 리턴 (모든 복잡한 분석 스킵)
                return None
            
            # 🔍 AI16Z 디버깅용 로그
            # ⚡ 스캔 모드시 디버그 출력 스킵
            if not self._scan_mode:
                self._write_debug_log(f"[DEBUG] [{clean_symbol}] analyze_symbol 시작 (일봉조건 ✅ 통과)")
            ...
            # ⚠️ 15분봉은 700봉 필요 (전략B 조건4: 700봉이내 MA480 5연속 하락 체크)
            ...
            # 🚫 1일봉 로드 완전 제거 (전략 C/D에서 사용하지 않음)
            # 전략 C: 3분봉, 5분봉만 사용
            # 전략 D: 5분봉, 15분봉만 사용
            # → 1일봉 불필요하므로 병목 제거를 위해 로드 생략
            df_1d = None
            rest_api_stats['success'].append('1d(불필요-생략)')

            if not self._scan_mode:
                self._write_debug_log(f"[DEBUG] [{clean_symbol}] WebSocket 데이터 조회 완료")

            # 핵심 데이터만 체크 (1일봉은 선택사항)
            critical_missing = []
            if df_3m is None or len(df_3m) < 3:
                critical_missing.append('3m')
            if df_5m is None or len(df_5m) < 3:
                critical_missing.append('5m')

            # 선택적 데이터 (로그용) - 1일봉은 완전 제외
            optional_missing = []
            if df_1m is None or len(df_1m) < 3:
                optional_missing.append('1m')
            if df_15m is None or len(df_15m) < 3:
                optional_missing.append('15m')
            # df_1d는 체크하지 않음 (전략에서 사용하지 않음)

            # 핵심 데이터 부족 시에만 스킵 (1일봉은 필수 아님)
            if critical_missing:
                # 첫 번째 실패 심볼에 대해서만 상세 로그 출력
                if not hasattr(self, '_first_rest_api_failure'):
                    self._first_rest_api_failure = True
                    self.logger.warning(f"[{clean_symbol}] 핵심 데이터 로드 실패: {critical_missing}")
                    ...
                    print(f"   실패: {rest_api_stats['failed'][:2]}")

                if not self._scan_mode:
                    self._write_debug_log(f"[DEBUG] [{clean_symbol}] 핵심 데이터 부족 - 스킵: {', '.join(critical_missing)}")
                return None

            # 선택적 데이터 부족은 경고만 (계속 진행)
            if optional_missing and not hasattr(self, '_optional_data_warning_shown'):
                self._optional_data_warning_shown = True
                print(f"ℹ️ 선택적 데이터 일부 누락 (계속 진행): {', '.join(optional_missing[:2])}")  # 처음 2개만
            ...
            if df_1m is not None and len(df_1m) > 0:
                ...
                try:
                    ...
                    # WebSocket 데이터에서 현재가 조회 시도
                    try:
                        ws_data = websocket_provider.get_ohlcv(symbol, '1m', 1)
                        if ws_data and len(ws_data) > 0:
                            current_price = float(ws_data[-1][4])  # close price
                        else:
                            current_price = None  # DCA 작업 건너뛰기용
                    except:
                        current_price = None  # DCA 작업 건너뛰기용

            # 디스플레이용 가격 설정 (DCA 작업과 분리)
            ...
            if strategy_3m_3rd_met:  # 전략 통과 (1 AND (2 OR 3) 구조)
                ...
                if is_data_insufficient_c:  # 데이터 부족인 경우
                    ...
                    self._write_debug_log(f"[DATA INSUFFICIENT] {symbol}: 3분봉 데이터 부족으로 전략A 조건 검사 불가")
            ...
            # 🔍 디버그: 전략A 상태 출력 (디버그 모드에서만)
            if not self._scan_mode or (hasattr(self, '_debug_print_enabled') and self._debug_print_enabled):
                print(f"🔍 [전략A] {symbol.replace('/USDT:USDT', '')}: failed={failed_count_preview}, status={status_3m_3rd}")

            if status_3m_3rd != 'no_signal':
                # 🔧 전략A는 2개 조건으로 수정 (조건1 + 조건2 + SuperTrend)
            ...
            if strategy_5m_4th_met_safe:  # 전략 통과 (5개 조건 모두 충족)
                ...
                self._write_debug_log(f"[DATA INSUFFICIENT] {symbol}: 5분봉 데이터 부족으로 전략B 조건 검사 불가")
            else:
                # 실패 개수에 따른 분류 (전략B는 5개 조건)
            ...
            if not is_data_insufficient and (status_5m_4th == 'near_entry' or status_5m_4th == 'potential_entry'):
                ...
                print(f"🔍 [전략B-{status_5m_4th.upper()}] {clean_name}: failed={failed_5m_4th}/5 | d1={d_conds.get('d1', '?')} d2={d_conds.get('d2', '?')} d3={d_conds.get('d3', '?')} d4={d_conds.get('d4', '?')} d5={d_conds.get('d5', '?')}")
            ...
            if status_5m_4th != 'no_signal':
                ...
                result_5m_4th = {
                    ...
                    'strategy_type': '전략B: 5분봉 초입 초강력 타점',
                    'total_conditions': 5,
                    'failed_count': actual_failed_for_display,
                    'conditions': conditions_5m_4th,
                    'conditions_summary': [f"5분봉B전략-{5-actual_failed_for_display}/5개 통과"],
                    'price': current_price,
                    'timestamp': current_time,
                    'change_24h': change_24h,
                    'daily_candle_change': daily_candle_change
    ...
    def scan_symbols(self, symbols):
        ...
        try:
            ...
            # 🚀 전체 심볼 목록 저장 (실시간 활용)
            if all_tickers:
                all_usdt_symbols = [s for s in all_tickers.keys() if s.endswith('/USDT:USDT')]
                self._recent_ticker_symbols = all_usdt_symbols
                print(f"✅ 티커 데이터 수집 완료: {len(tickers_cache)}개/{len(symbols)}개")
                print(f"📊 전체 USDT 심볼 발견: {len(all_usdt_symbols)}개")
            else:
                print(f"✅ 티커 데이터 수집 완료: {len(tickers_cache)}개/{len(symbols)}개")
        except Exception as e:
        ...
        if hasattr(self, '_speed_test_mode') and self._speed_test_mode:
            ...
            max_workers = min(len(symbols), 15)  # 최대 15개 동시 처리 (IP 밴 방지)
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                if cache_size >= expected_cache_entries * 0.8:
                    ...
                    # API 호출이 많을 때 예상 소요시간 안내
                    if expected_api_calls > 1000:
                        estimated_minutes = (expected_api_calls / 150)  # 분당 약 150개 API 호출 가능
                        print(f"⏱️ 첫 스캔 예상 소요시간: {estimated_minutes:.1f}분 (다음 스캔부터 10초 이내)")
                else:
                    print(f"🔄 첫 스캔: 데이터 수집 중 (캐시 구축), 약 10-15분 소요 예상")
                ...
                for future, symbol in submitted_futures:
                    completed_count += 1
                    # 진행 상황을 더 자주 표시 (50개마다)
                    if completed_count % 50 == 0 or completed_count == len(submitted_futures):
                        elapsed = time.time() - start_time
                        rate = completed_count / elapsed if elapsed > 0 else 0
                        remaining = len(submitted_futures) - completed_count
                        eta_seconds = remaining / rate if rate > 0 else 0
                        eta_minutes = eta_seconds / 60
                        print(f"⚡ 진행 중: {completed_count}/{len(submitted_futures)} ({rate:.1f}/s) ETA: {eta_minutes:.1f}분", end='\r')
        ...
        if entry_signals:
            ...
            for result in entry_signals:
                strategy_type = result.get('strategy_type', '전략A: 3분봉 시세 초입 포착')
                if strategy_type not in strategy_groups:
                    strategy_groups[strategy_type] = []
                strategy_groups[strategy_type].append(result)

            # 전략별로 출력 (A+B+C → A → B → C 순서)
            strategy_order = ['전략A+B+C: 3분봉+5분봉+15분봉 복합 진입', '전략A: 3분봉 시세 초입 포착', '전략B: 5분봉 초입 초강력 타점', '전략C: 15분봉 대박파동 맥점']
            ...
            print(f"\n[SIGNAL] 진입신호 [전략A: 3분봉 시세 초입 포착]")
            print("   없음")

            print(f"\n[SIGNAL] 진입신호 [전략B: 5분봉 초입 초강력 타점]") 
            print("   없음")
            
            print(f"\n[SIGNAL] 진입신호 [전략C: 15분봉 대박파동 맥점]")
            print("   없음")
        ...
        if near_by_symbol:
            ...
            for strategy_type in strategy_order:
                ...
                for symbol, result in symbol_results:
                    ...
                    elif strategy_type == '전략B: 5분봉 초입 초강력 타점':
                    ...
                    elif strategy_type == '전략A: 3분봉 시세 초입 포착':
                        total_conditions = result.get('total_conditions', 6)
                        failed_count = result.get('failed_count', 0)
            ...
            # 활성화된 전략만 표시 (A전략 → B전략 → C전략 순서)
            print(f"\n진입임박 [전략A: 3분봉 시세 초입 포착] (1개 조건 미충족)")
            print("   없음")
            print(f"\n진입임박 [전략B: 5분봉 초입 초강력 타점] (1개 조건 미충족)")
            print("   없음")
            print(f"\n진입임박 [전략C: 15분봉 대박파동 맥점] (1개 조건 미충족)")
            print("   없음")
        ...
        if potential_entry:
            ...
            # 모든 전략을 개별적으로 표시 (A전략 → B전략 → C전략 순서)
            print(f"\n진입확률 [전략A: 3분봉 시세 초입 포착] (2개 조건 미충족)")
            print("   없음")
            print(f"\n진입확률 [전략B: 5분봉 초입 초강력 타점] (2개 조건 미충족)")
            print("   없음")
            print(f"\n진입확률 [전략C: 15분봉 대박파동 맥점] (2개 조건 미충족)")
            print("   없음")
        ...
        if watchlist:
            ...
            for strategy in strategy_order:
                ...
                total_counts = [result.get('total_conditions', 3 if 'A:' in strategy else 5) for result in items]
                
                # 대표값 계산 (가장 많은 유형)
                avg_failed = sum(failed_counts) / len(failed_counts) if failed_counts else 0
                avg_total = sum(total_counts) / len(total_counts) if total_counts else (3 if 'A:' in strategy else 5)
    ...
    def _get_strategy_info(self, symbol):
        ...
        try:
            ...
            if hasattr(self, '_last_analysis_results') and symbol in self._last_analysis_results:
                strategy_type = self._last_analysis_results[symbol].get('strategy_type', '전략A: 3분봉 시세 초입 포착')
                print(f"[전략조회] {clean_symbol} → {strategy_type} (저장된 값)")
                return strategy_type
            else:
                # 기본값 반환
                print(f"[전략조회] {clean_symbol} → 전략A: 3분봉 시세 초입 포착 (기본값 - 저장된 값 없음)")
                ...
                return '전략A: 3분봉 시세 초입 포착'
        except Exception as e:
            clean_symbol = symbol.replace('/USDT:USDT', '').replace('/USDT', '')
            print(f"[전략조회] {clean_symbol} → 오류: {e}")
            return '전략A: 3분봉 시세 초입 포착'

    def send_unified_entry_alert(self, symbol, entry_price, quantity, entry_amount, is_dca=False, strategy_info='전략A: 3분봉 시세 초입 포착'):
        ...
        try:
            ...
            # 전략 정보 상세 표시
            strategy_display = {
                '전략A: 3분봉 시세 초입 포착': '🎯 전략A: 3분봉 시세 초입 포착\n   (복합 논리 조건 AND 5분봉 SuperTrend)',
                '전략B: 5분봉 초입 초강력 타점': '🎯 전략B: 5분봉 초입 초강력 타점\n   (5개 조건 모두 충족 AND 5분봉 SuperTrend)',
                '전략C: 15분봉 대박파동 맥점': '🎯 전략C: 15분봉 대박파동 맥점\n   (3개 복합 조건 모두 충족)',
                '전략A+B+C: 3분봉+5분봉+15분봉 복합 진입': '🎯 전략A+B+C: 3분봉+5분봉+15분봉 복합 진입\n   (전략A, B, C 중 하나 이상 충족된 강력한 시그널)',
                '3분봉전략': '🎯 3분봉 전략'
    ...
    def send_trade_failure_alert(self, symbol, failure_reason):
        ...
        try:
            ...
            # 실패 알림 전송 (HTML 특수 문자 이스케이프 처리)
            kst_time = get_korea_time().strftime('%H:%M:%S')
            
            # HTML 특수 문자 이스케이프 처리
            safe_failure_reason = str(failure_reason).replace('<', '&lt;').replace('>', '&gt;').replace('&', '&amp;')
            
            message = f"❌ [진입 실패] {clean_symbol}\n"
            message += f"━━━━━━━━━━━━━━━━━━━━━━\n"
            message += f"🚫 실패 사유: {safe_failure_reason}\n"
    ...
    def execute_trade(self, symbol, entry_price):
        ...
        try:
            ...
            # 🔧 타입 안전성 확보: entry_price를 float로 강제 변환
            try:
                entry_price = float(entry_price)
                print(f"[거래실행] 📊 진입가: ${entry_price:.6f}")
            except (ValueError, TypeError) as e:
                print(f"[거래실행] ❌ 진입가 타입 오류: {entry_price} ({type(entry_price)}) - {e}")
                return False

            # 🔄 실시간 포지션 동기화 (수동 청산 반영) - 조용한 모드
            self.sync_positions_with_exchange(quiet=True)

            # 포지션 수 불일치 감지시 강제 동기화
            current_positions = len(self.active_positions)
            if current_positions > 10:  # 비정상적으로 많은 포지션 감지
                print(f"[포지션확인] ⚠️ 비정상적 포지션 수 감지: {current_positions}개 - 강제 동기화 실행")
                current_positions = self.force_sync_positions_with_exchange()
            ...
            try:
                # IP 밴 체크
                if hasattr(self, '_ip_ban_detected') and self._ip_ban_detected:
                    print(f"[거래실행] 🚨 IP 밴으로 잔고 확인 건너뜀 - 거래 중단")
                    return False
                
                balance = self.exchange.fetch_balance()
                # 🔧 타입 안전성: USDT 잔고 강제 float 변환
                try:
                    usdt_balance = float(balance['USDT']['free'])
                    if usdt_balance < 0:
                        usdt_balance = 0.0
                except (ValueError, TypeError, KeyError) as balance_error:
                    print(f"[거래실행] ❌ USDT 잔고 타입 변환 실패: {balance.get('USDT', {}).get('free')} ({type(balance.get('USDT', {}).get('free'))}) - {balance_error}")
                    return False
                
                # 잔고 캐싱
                self._cache_balance(usdt_balance)
                
                print(f"[거래실행] 💵 USDT 잔고: ${usdt_balance:.2f}")
            except Exception as e:
                error_str = str(e).lower()
                if "418" in str(e) or "429" in str(e) or "banned" in error_str:
                    print(f"[거래실행] 🚨🚨🚨 IP 밴 감지 - 거래 중단 🚨🚨🚨")
                    self._ip_ban_detected = True
                    return False
            ...
            # 🚀 최소 주문 금액 확인 및 조정 (타입 안전성 강화)
            markets = self._get_cached_markets()
            market = markets.get(symbol)
            min_cost = float(self.min_order_amount)  # $6 (기본 $5 + 안전마진 $1)
            
            if market and 'limits' in market and 'cost' in market['limits'] and market['limits']['cost']['min']:
                try:
                    min_cost_raw = market['limits']['cost']['min']
                    if min_cost_raw is not None:
                        min_cost = float(min_cost_raw)
                except (ValueError, TypeError, AttributeError):
                    min_cost = float(self.min_order_amount)  # 폴백값 사용
            
            # 타입 안전성: position_value도 float 확인
            try:
                position_value = float(position_value)
                min_cost = float(min_cost)
            except (ValueError, TypeError):
                print(f"[거래실행] ❌ 타입 변환 실패: position_value={position_value}, min_cost={min_cost}")
                return False
            ...
            # 🔧 타입 안전성 확보: current_price를 float로 강제 변환
            try:
                current_price = float(current_price)
            except (ValueError, TypeError) as e:
                print(f"[거래실행] ❌ 현재가 타입 오류: {current_price} ({type(current_price)}) - {e}")
                return False
                
            quantity = position_value / current_price
            
            # 🔧 타입 안전성: min_order_amount 타입 확인
            try:
                min_order_amount = float(self.min_order_amount)
            except (ValueError, TypeError):
                min_order_amount = 6.0  # 기본값
            
            # 실제 주문 금액 검증 (최종 안전장치)
            actual_order_value = quantity * current_price
            if actual_order_value < min_order_amount:
                print(f"[거래실행] ⚠️ 최종 주문금액 미달: ${actual_order_value:.2f} < ${min_order_amount:.2f}")
                # 강제로 최소 주문금액으로 조정
                quantity = min_order_amount / current_price
                actual_order_value = min_order_amount
            ...
            if self.dca_manager:
                try:
                    # 🔧 타입 안전성: DCA 호출 전 모든 매개변수 타입 강제 변환
                    try:
                        safe_symbol = str(symbol) if symbol is not None else ""
                        safe_entry_price = float(entry_price) if entry_price is not None else 0.0
                        safe_balance = float(usdt_balance) if usdt_balance is not None else 0.0
                        safe_leverage = float(self.leverage) if hasattr(self, 'leverage') and self.leverage is not None else 10.0
                        
                        print(f"[DCA진입] 🔧 타입 변환 완료: symbol={safe_symbol}, entry_price={safe_entry_price}, balance={safe_balance}, leverage={safe_leverage}")
                        
                        # 값 검증
                        if safe_entry_price <= 0:
                            print(f"[DCA진입] ❌ 잘못된 진입가: {safe_entry_price}")
                            return False
                        if safe_balance <= 0:
                            print(f"[DCA진입] ❌ 잘못된 잔고: {safe_balance}")  
                            return False
                            
                    except (ValueError, TypeError) as convert_error:
                        print(f"[DCA진입] ❌ 매개변수 타입 변환 실패: symbol={symbol} ({type(symbol)}), entry_price={entry_price} ({type(entry_price)}), balance={usdt_balance} ({type(usdt_balance)}), leverage={getattr(self, 'leverage', None)} - {convert_error}")
                        return False
                    
                    # DCA 시스템으로 최초 진입 처리 (타입 안전한 매개변수 사용)
            ...
            try:
                ...
                print(f"[기존방식] ❌ 현재가 타입 오류: {current_price} ({type(current_price)}) - {e}")
                return False
            
            # 다시 한번 최소 주문 금액 확인 (DCA fallback시 안전장치)
            # 🔧 타입 안전성: min_order_amount 재확인
            try:
                min_order_amount_fallback = float(self.min_order_amount)
            except (ValueError, TypeError):
                min_order_amount_fallback = 6.0
                
            # 🔧 타입 안전성: 모든 값을 float으로 변환 후 계산
            try:
                final_order_value = float(quantity) * float(current_price)
                min_order_fallback = float(min_order_amount_fallback)
            except (ValueError, TypeError) as e:
                print(f"[기존방식] ❌ 타입 변환 실패: {e}")
                return False
                
            if final_order_value < min_order_fallback:
                print(f"[기존방식] ⚠️ DCA fallback 최종 검증: ${final_order_value:.2f} < ${min_order_fallback}")
                # 최소 주문 금액으로 강제 조정
                quantity = min_order_fallback / float(current_price)
                final_order_value = min_order_fallback
            ...
            try:
                ...
                min_amount_raw = amount_limits.get('min', 0)

                # 🔧 타입 안전성: min_amount도 float로 변환
                try:
                    min_amount = float(min_amount_raw) if min_amount_raw else 0
                except (ValueError, TypeError):
                    print(f"[기존방식] ⚠️ 최소 수량 타입 오류: {min_amount_raw} ({type(min_amount_raw)})")
                    min_amount = 0

                # 🔧 타입 안전성: 비교 전 두 값 모두 float 확인
                if min_amount and float(quantity) < float(min_amount):
            ...
            try:
                ...
                try:
                    ...
                    print(f"[기존방식] ❌ 최종 타입 변환 실패: quantity={quantity} ({type(quantity)}), current_price={current_price} ({type(current_price)}) - {type_error}")
                    return False
            ...
            if not exit_signal and profit_pct < 0:
                ...
                if self.dca_manager:
                    ...
                    if clean_symbol in self.dca_manager.positions:
                        ...
                        # 초기 진입가 기준 -10% 고정 손절
                        stop_loss_pct = -10.0  # 모든 단계에서 -10% 고정
    ...
    def _execute_entry_signal(self, signal_data: dict):
        ...
        try:
            ...
            try:
                ...
                change_24h = float(signal_data.get('change_24h', 0))
            except (ValueError, TypeError) as e:
                print(f"❌ {clean_symbol}: 데이터 타입 오류 - current_price={signal_data.get('current_price')}, change_24h={signal_data.get('change_24h')} - {e}")
                return
            ...
            try:
                ...
                if self.dca_manager:
                    try:
                        # 잔고 계산 (position_size_pct * 잔고)
                        available_balance = 1000.0  # WebSocket 모드용 기본값
                        entry_amount = available_balance * self.position_size_pct
                        
                        # 🔧 타입 안전성: 모든 매개변수 타입 강제 변환
                        try:
                            safe_symbol = str(clean_symbol) if clean_symbol is not None else ""
                            safe_entry_price = float(current_price) if current_price is not None else 0.0
                            safe_balance = float(entry_amount) if entry_amount is not None else 0.0
                            safe_leverage = float(self.leverage) if hasattr(self, 'leverage') and self.leverage is not None else 10.0
                            
                            print(f"[WebSocket진입] 🔧 타입 변환 완료: symbol={safe_symbol}, entry_price={safe_entry_price}, balance={safe_balance}, leverage={safe_leverage}")
                            
                            # 값 검증
                            if safe_entry_price <= 0:
                                print(f"[WebSocket진입] ❌ 잘못된 진입가: {safe_entry_price}")
                                return
                            if safe_balance <= 0:
                                print(f"[WebSocket진입] ❌ 잘못된 잔고: {safe_balance}")
                                return
                                
                        except (ValueError, TypeError) as convert_error:
                            print(f"[WebSocket진입] ❌ 매개변수 타입 변환 실패: symbol={clean_symbol} ({type(clean_symbol)}), entry_price={current_price} ({type(current_price)}), balance={entry_amount} ({type(entry_amount)}), leverage={getattr(self, 'leverage', None)} - {convert_error}")
                            return
                        
                        entry_result = self.dca_manager.enter_new_position(
                            symbol=safe_symbol,
                            entry_price=safe_entry_price,
                            balance=safe_balance,
                            leverage=safe_leverage
                        ...
                        if entry_result and entry_result.get('success'):
                            ...
                            # 기존 통합 진입 알림 시스템 사용 (DCA 정보 포함)
                            try:
                                self.send_unified_entry_alert(
                                    symbol=symbol,
                                    entry_price=current_price,
                                    leverage=10.0,  # 기본 레버리지
                                    notional_amount=None,  # DCA 매니저에서 계산
                                    strategy_info="WebSocket 전용 모드",
                                    change_24h=change_24h
                                )
                            except Exception as e:
                                print(f"통합 진입 알림 실패: {e}")
                        else:
    ...
    def monitor_positions_realtime(self):
        ...
        current_time = time.time()
        if current_time - self._last_sync_time > 300:  # 🚨 5분으로 확대 (Rate Limit 차단)
            # Rate Limit 상황에서는 동기화 스킵
            print("🚨 Rate Limit 상황 - 포지션 동기화 스킵")
        ...
        if current_time - self._last_dca_trigger_check > 30:  # 30초마다 체크 (API 호출 최소화)
            if self.dca_manager and hasattr(self.dca_manager, 'positions'):
                try:
                    # 각 DCA 포지션의 트리거 확인
                ...
                try:
                    ...
                    for symbol in list(self.active_positions.keys()):
                        if symbol in self.dca_manager.positions:
                            ...
                            try:
                                # IP 밴 체크 (지정가 주문만 건너뛰고 손절은 계속)
                                ip_ban_detected = hasattr(self, '_ip_ban_detected') and self._ip_ban_detected
                                if ip_ban_detected:
                                    print(f"🚨 IP 밴 상태 - 지정가 주문만 건너뜀 (손절은 계속)")
                                    # continue 제거 - 손절은 계속 실행되도록
                                
                                # 🚀 잔고 캐싱 (5분 캐시) - IP 밴이 아닐 때만
                                if not ip_ban_detected:
                                    total_balance = self._get_cached_balance()
                                    if total_balance is None:
                                        total_balance = 1000.0  # IP 밴시 기본값 사용
                                    
                                    # 유효한 가격이 있을 때만 지정가 주문 체크
                                    if current_price is not None and current_price > 0:
                                        self.dca_manager.check_pending_limit_orders(symbol, current_price, total_balance)
                                else:
                                    # IP 밴시에도 손절을 위해 기본 잔고값 사용
                                    total_balance = 1000.0
                            except Exception as limit_check_error:
                                pass  # 조용히 실패

                            # DCA 트리거 체크 (최대 수익률 업데이트 및 5% 수익 DCA 비활성화 포함)
                            # 🚨 중요: 손절을 위해 가격 정보가 없어도 거래소에서 직접 조회 시도
                            if current_price is not None and current_price > 0:
                                self.dca_manager.check_dca_triggers(symbol, current_price)
                            else:
                                # 가격 정보 없음 - 거래소에서 직접 조회해서 손절 체크
                                try:
                                    print(f"[손절시도] ⚠️ {symbol.replace('/USDT:USDT', '')} 가격 정보 없음 - 거래소 직접 조회 시도")
                                    ticker = self.exchange.fetch_ticker(symbol)
                                    fallback_price = float(ticker['last'])
                                    print(f"[손절시도] ✅ 거래소 가격 조회 성공: ${fallback_price:.6f}")
                                    self.dca_manager.check_dca_triggers(symbol, fallback_price)
                                except Exception as ticker_error:
                                    print(f"[손절시도] ❌ 거래소 가격 조회도 실패: {ticker_error}")
                                    # 최후의 수단: DCA 매니저에 None을 전달해서 내부에서 처리하게 함
                                    try:
                                        self.dca_manager.check_dca_triggers(symbol, None)
                                    except:
                                        pass
                    ...
                    # 🚨 중요: 손절 시스템 실패를 로깅 (조용한 실패 금지)
                    print(f"[손절체크] ❌ DCA/손절 체크 실패: {dca_check_error}")
                    self.logger.error(f"DCA/손절 체크 실패: {dca_check_error}")
                    import traceback
                    traceback.print_exc()
    ...
    def print_daily_stats(self):
        ...
        try:
            ...
            try:
                # IP 밴 체크 - API 호출 차단
                if hasattr(self, '_ip_ban_detected') and self._ip_ban_detected:
                    print("   🚨 IP 밴으로 인해 전체 수익률 계산 건너뜀")
                    return
                
                # 현재 잔고 조회 (캐시 우선)
                current_usdt_balance = self._get_cached_balance()
                if current_usdt_balance is None:
                    # 캐시 없으면 건너뛰기 (API 호출 방지)
                    print("   ⚠️ 잔고 캐시 없음 - 수익률 계산 건너뜀")
                    return
    ...
    def _get_cached_balance(self):
        """
        캐시된 잔고 조회 (리팩토링: cache_manager 사용)
        """
        if HAS_CACHE_MANAGER and self.cache_manager:
            return self.cache_manager.get_cached_balance()
        else:
            # 폴백: 기존 로직
            try:
    ...
    def _cache_balance(self, balance):
        """
        잔고 캐싱 (리팩토링: cache_manager 사용)
        """
        if HAS_CACHE_MANAGER and self.cache_manager:
    ...
    def _wrap_exchange_methods(self):
        """🛡️ Exchange API 메서드를 IP 밴 안전 모드로 래핑"""
        try:
            # 원본 메서드 백업
            self._original_fetch_balance = self.exchange.fetch_balance
            self._original_fetch_positions = self.exchange.fetch_positions
            self._original_fetch_ticker = self.exchange.fetch_ticker
            self._original_fetch_tickers = self.exchange.fetch_tickers
            self._original_fetch_markets = self.exchange.fetch_markets
            self._original_fetch_ohlcv = self.exchange.fetch_ohlcv
            
            # 안전한 래퍼로 교체
            self.exchange.fetch_balance = self._safe_fetch_balance
            self.exchange.fetch_positions = self._safe_fetch_positions
            self.exchange.fetch_ticker = self._safe_fetch_ticker
            self.exchange.fetch_tickers = self._safe_fetch_tickers
            self.exchange.fetch_markets = self._safe_fetch_markets
            self.exchange.fetch_ohlcv = self._safe_fetch_ohlcv
            
            print("🛡️ Exchange API 래퍼 활성화 - IP 밴 완전 방지")
            
        except Exception as e:
            print(f"⚠️ Exchange 래퍼 설정 실패: {e}")
    
    def _safe_fetch_balance(self, *args, **kwargs):
        """IP 밴 안전 잔고 조회 (캐시 우선)"""
        if hasattr(self, '_ip_ban_detected') and self._ip_ban_detected:
            # IP 밴 상황에서는 캐시된 잔고 반환
            cached_balance = self._get_cached_balance()
            if cached_balance is not None:
                print(f"💾 캐시된 잔고 사용: ${cached_balance:.2f}")
                return {'USDT': {'total': cached_balance, 'free': cached_balance, 'used': 0}}
            else:
                # 캐시도 없으면 기본값 반환
                print("⚠️ 캐시된 잔고 없음 - 기본값 사용")
                return {'USDT': {'total': 50.0, 'free': 50.0, 'used': 0}}
        
        try:
            result = self._original_fetch_balance(*args, **kwargs)
            # 성공시 캐싱
            if 'USDT' in result:
                self._cache_balance(result['USDT']['total'])
            return result
        except Exception as e:
            if "418" in str(e) or "429" in str(e) or "banned" in str(e).lower():
                print("🚨🚨🚨 IP 밴 감지 - 캐시 모드로 전환 🚨🚨🚨")
                self._ip_ban_detected = True
                # 즉시 캐시된 데이터 반환
                cached_balance = self._get_cached_balance()
                if cached_balance is not None:
    ...
    def _safe_fetch_positions(self, *args, **kwargs):
        """IP 밴 안전 포지션 조회 (로컬 추적 데이터 사용)"""
        if hasattr(self, '_ip_ban_detected') and self._ip_ban_detected:
            print("💾 IP 밴 상황 - 로컬 추적 포지션 데이터 사용")
            return self._get_local_positions()
        
        try:
            result = self._original_fetch_positions(*args, **kwargs)
            # 성공시 로컬 데이터 업데이트
            self._update_local_positions(result)
            return result
        except Exception as e:
            if "418" in str(e) or "429" in str(e) or "banned" in str(e).lower():
                print("🚨🚨🚨 IP 밴 감지 - 로컬 추적 모드로 전환 🚨🚨🚨")
                self._ip_ban_detected = True
                return self._get_local_positions()
            raise e
    
    def _safe_fetch_ticker(self, *args, **kwargs):
        """IP 밴 안전 티커 조회"""
        if hasattr(self, '_ip_ban_detected') and self._ip_ban_detected:
            raise Exception("IP_BAN_DETECTED - API 호출 차단")
        
        try:
            return self._original_fetch_ticker(*args, **kwargs)
        except Exception as e:
            if "418" in str(e) or "429" in str(e) or "banned" in str(e).lower():
                print("🚨🚨🚨 IP 밴 감지 - 모든 API 차단 🚨🚨🚨")
                self._ip_ban_detected = True
            raise e
    
    def _safe_fetch_tickers(self, *args, **kwargs):
        """IP 밴 안전 티커 목록 조회"""
        if hasattr(self, '_ip_ban_detected') and self._ip_ban_detected:
            raise Exception("IP_BAN_DETECTED - API 호출 차단")
        
        try:
            return self._original_fetch_tickers(*args, **kwargs)
        except Exception as e:
            if "418" in str(e) or "429" in str(e) or "banned" in str(e).lower():
                print("🚨🚨🚨 IP 밴 감지 - 모든 API 차단 🚨🚨🚨")
                self._ip_ban_detected = True
    ...
    def _safe_fetch_markets(self, *args, **kwargs):
        """IP 밴 안전 마켓 조회"""
        if hasattr(self, '_ip_ban_detected') and self._ip_ban_detected:
            raise Exception("IP_BAN_DETECTED - API 호출 차단")
        
        try:
            return self._original_fetch_markets(*args, **kwargs)
        except Exception as e:
            if "418" in str(e) or "429" in str(e) or "banned" in str(e).lower():
                print("🚨🚨🚨 IP 밴 감지 - 모든 API 차단 🚨🚨🚨")
                self._ip_ban_detected = True
    ...
    def _safe_fetch_ohlcv(self, *args, **kwargs):
        """IP 밴 안전 OHLCV 조회"""
        if hasattr(self, '_ip_ban_detected') and self._ip_ban_detected:
            print("🚨 IP 밴으로 OHLCV 조회 건너뜀")
            return []
        
        try:
            return self._original_fetch_ohlcv(*args, **kwargs)
        except Exception as e:
            if "418" in str(e) or "429" in str(e) or "banned" in str(e).lower():
                print("🚨🚨🚨 IP 밴 감지 - 모든 API 차단 🚨🚨🚨")
                self._ip_ban_detected = True
    ...
    def _get_local_positions(self):
        """로컬 추적 포지션 데이터 반환"""
        try:
            if not hasattr(self, '_local_positions_cache'):
                self._local_positions_cache = []
            
            # DCA 매니저에서 포지션 정보 가져오기
            if hasattr(self, 'dca_manager') and self.dca_manager:
                local_positions = []
                
                # DCA 매니저의 활성 포지션을 ccxt 포지션 형식으로 변환
                for symbol, position_data in self.dca_manager.positions.items():
                    if position_data.get('status') == 'active':
                        local_positions.append({
                            'symbol': symbol,
                            'side': 'long',  # 현재 전략은 롱만 사용
                            'size': position_data.get('total_quantity', 0),
                            'contracts': position_data.get('total_quantity', 0),
                            'entryPrice': position_data.get('average_price', 0),
                            'markPrice': position_data.get('current_price', 0),
                            'unrealizedPnl': position_data.get('unrealized_pnl', 0),
                            'percentage': position_data.get('percentage', 0),
                            'timestamp': position_data.get('entry_time', 0)
                        })
                
                print(f"💾 로컬 추적 포지션: {len(local_positions)}개")
                return local_positions
            
            return self._local_positions_cache
            
        except Exception as e:
            print(f"⚠️ 로컬 포지션 조회 실패: {e}")
            return []
    
    def _update_local_positions(self, positions):
        """API 결과로 로컬 포지션 캐시 업데이트"""
        try:
            self._local_positions_cache = positions
            print(f"💾 로컬 포지션 캐시 업데이트: {len(positions)}개")
        except Exception as e:
            print(f"⚠️ 로컬 포지션 캐시 업데이트 실패: {e}")
    ...
    def force_sync_positions_with_exchange(self):
        """
        강제 포지션 동기화 - Rate Limit 무시하고 실제 거래소 데이터로 완전 동기화
        """
        try:
            # Rate limit 플래그 임시 해제
            original_rate_limited = getattr(self, '_api_rate_limited', False)
            self._api_rate_limited = False
            
            print(f"[강제동기화] 🔄 거래소 포지션 강제 새로고침 시작...")
            
            # 거래소에서 실제 포지션 조회
            positions = self.exchange.fetch_positions()
            
            # 기존 로컬 포지션 완전 초기화
            old_count = len(self.active_positions) if hasattr(self, 'active_positions') else 0
            self.active_positions = {}
            self.position_stats = {}
            ...
            actual_positions = {}
            for position in positions:
                if position['contracts'] > 0 and position['symbol'].endswith('/USDT:USDT'):
            ...
            # 새로운 포지션 데이터로 업데이트
            self.active_positions = actual_positions
            
            print(f"[강제동기화] ✅ 완료: {old_count}개 → {len(self.active_positions)}개")
            
            # DCA 시스템과도 강제 동기화
            if self.dca_manager and hasattr(self.dca_manager, 'force_sync_with_exchange'):
                self.dca_manager.force_sync_with_exchange(actual_positions)
            
            # Rate limit 플래그 복원
            self._api_rate_limited = original_rate_limited
            
            return len(self.active_positions)
            
        except Exception as e:
            print(f"[강제동기화] ❌ 오류 발생: {e}")
            # Rate limit 플래그 복원
            if 'original_rate_limited' in locals():
                self._api_rate_limited = original_rate_limited
            return len(self.active_positions) if hasattr(self, 'active_positions') else 0

    def start_periodic_position_sync(self):
        """5초마다 포지션 동기화를 위한 백그라운드 작업 시작"""
        def sync_worker():
            while getattr(self, '_running', True):
                try:
                    ...
                    if hasattr(self.exchange, 'apiKey') and self.exchange.apiKey:
                        ...
                        # 불일치 감지시 강제 동기화
                        if hasattr(self, 'active_positions') and len(self.active_positions) > self.max_positions:
                            print(f"[주기동기화] ⚠️ 포지션 수 이상 감지 - 강제 동기화")
                            self.force_sync_positions_with_exchange()
                            
                except Exception as e:
                    # 동기화 오류 무시 (로그만 남기고 계속 실행)
                    if hasattr(self, 'logger'):
                        self.logger.error(f"주기적 포지션 동기화 오류: {e}")
    ...
    def sync_positions_with_exchange(self, quiet=False):
        ...
        try:
            ...
            if hasattr(self, '_api_rate_limited') and self._api_rate_limited:
                # Rate Limit 시간이 5분 이상 경과했으면 재시도
                if hasattr(self, '_last_rate_limit_check'):
                    if time.time() - self._last_rate_limit_check > 300:  # 5분
                        print(f"[포지션동기화] 🔄 Rate Limit 시간 경과 - 재시도")
                        self._api_rate_limited = False
                    else:
                        print(f"[포지션동기화] ⏰ Rate Limit 대기 중 - 캐시 데이터 사용")
                        # 캐시 데이터 표시만 하고 실제 동기화는 건너뛰기
                        if hasattr(self, 'active_positions'):
                            print(f"📊 [캐시포지션] {len(self.active_positions)}개")
                        return
                else:
            ...
            try:
                positions = self.exchange.fetch_positions()
                # quiet 모드가 아닐 때만 진행 상황 출력
                if not quiet and any(pos['contracts'] > 0 for pos in positions):
    ...
    def _detailed_local_sync(self, active_exchange_positions, sync_stats):
        ...
        for symbol, exchange_pos in active_exchange_positions.items():
            ...
            if symbol in self.active_positions:
                ...
                # 수동 포지션은 신규 포지션으로 카운트하지 않음
                if not hasattr(self, '_detected_manual_positions'):
                    self._detected_manual_positions = set()
                
                if symbol in self._detected_manual_positions:
                    sync_details['status'] = 'manual_detected'
                    # 수동 포지션은 new_positions에 카운트하지 않음
                    self.logger.debug(f"[로컬동기화] 🔍 {symbol.replace('/USDT:USDT', '')} 수동 포지션 감지됨 (신규 카운트 제외)")
                else:
    ...
    def _update_position_states(self, active_exchange_positions, sync_stats):
        ...
        for symbol, position_data in active_exchange_positions.items():
            # 🚨 중요 수정: 기존 추적 중인 포지션만 업데이트 (신규 자동 추가 방지)
            if symbol in self.active_positions:
                # 기존 포지션 상태만 업데이트
                self.active_positions[symbol] = position_data
                updated_positions = True
            else:
                # 수동 포지션은 조용히 추적만 하고 출력하지 않음
                self._detected_manual_positions.add(symbol)
                continue  # active_positions에 추가하지 않음
        
        # 실제 업데이트된 포지션이 있을 때만 메시지 출력
    ...
    def _handle_missing_positions(self, active_exchange_positions, sync_stats):
        ...
        for symbol in active_exchange_positions:
            if symbol not in self.active_positions:
                ...
                # 새로 발견된 수동 포지션인지 확인 (이미 _update_position_states에서 추가된 것은 제외)
                if symbol not in self._detected_manual_positions:
                    new_manual_positions.append(symbol)
                    self._detected_manual_positions.add(symbol)
        
        # 새로 발견된 수동 포지션이 있을 때만 출력 (최초 1회만)
        if new_manual_positions:
            print(f"[누락포지션] ⚠️ {len(new_manual_positions)}개 새로운 수동 포지션 감지:")
            for symbol in new_manual_positions:
                symbol_name = symbol.replace('/USDT:USDT', '')
                position_data = active_exchange_positions[symbol]
                
                print(f"   🔍 {symbol_name}: ${position_data['entry_price']:.4f}, "
                      f"{position_data['side']}, {abs(position_data['quantity']):.4f} (수동 거래)")
                
                missing_positions_report.append({
                    'symbol': symbol,
                    'action': 'detected_only',  # 감지만 함 (추가 안함)
                    'position_data': position_data
                })
            
            # sync_stats에는 새로 발견된 것만 카운트
            sync_stats['manual_positions_detected'] = len(new_manual_positions)
            print(f"[누락포지션] 📋 새로운 수동 거래 포지션 {len(new_manual_positions)}개 감지됨 (A/B/C 전략 외 진입)")
        else:
            # 새로운 수동 포지션이 없으면 0으로 설정
            sync_stats['manual_positions_detected'] = 0
        
        # 포지션이 사라진 경우 감지 목록에서 제거
        removed_manual_positions = self._detected_manual_positions - set(active_exchange_positions.keys())
        if removed_manual_positions:
            self._detected_manual_positions -= removed_manual_positions
    ...
    def _generate_sync_summary_report(self, sync_stats, detailed_reports, quiet=False):
        ...
        if quiet:
            ...
            # 새로 감지된 수동 포지션이 있는 경우에만 출력 (0개면 출력 안함)
            elif sync_stats.get('manual_positions_detected', 0) > 0:
                print(f"📊 새로운 수동 포지션 {sync_stats['manual_positions_detected']}개 감지")
            # 아무 변경사항이 없으면 완전히 조용함
            return
    ...
    def _apply_integrated_filtering(self, candidate_symbols):
        ...
        try:
            ...
            return self._get_top_symbols(candidate_symbols, limit=100)

    def _apply_4h_filtering(self, candidate_symbols):
        """🚀 IP 밴 방지: 4h 필터링 완전 비활성화 - 모든 API 호출 차단"""
        print("🚫 IP 밴 방지: 4h 필터링 완전 비활성화 (0개 API 호출)")
        return candidate_symbols  # 모든 심볼 그대로 반환 - API 호출 없음
    
    def _apply_4h_filtering_original(self, candidate_symbols):
            ...
            for idx, symbol_data in enumerate(batch_symbols):
                try:
                    ...
                    for i in range(-4, 0):
                        ...
                        if open_price > 0:
                            surge_pct = ((high_price - open_price) / open_price) * 100
                            if surge_pct >= 3.0:  # 3% 급등 조건 (문서와 일치)
                                surge_found = True
                                break
                    ...
                    overall_rise = False
                    if len(ohlcv) >= 5:
                        start_price = ohlcv[-5][1]  # 4봉 전 시가 (5번째 전 캔들)
                        end_price = ohlcv[-1][4]    # 0봉 종가 (현재 캔들)
                        
                        if start_price > 0:
                            overall_rise_pct = ((end_price - start_price) / start_price) * 100
                            overall_rise = overall_rise_pct >= 0.0  # 0% 이상 상승

                    # 조건 3: 09:00~현재까지 상승률 0% 이상
                    daily_rise = False
                    try:
                        from datetime import datetime, timezone, timedelta
                        ...
                        # 오늘 09:00 KST = 00:00 UTC
                        today_09_kst = now_utc.replace(hour=0, minute=0, second=0, microsecond=0)
                        
                        # 09:00 KST 이후 1분봉 데이터로 상승률 계산
                        df_1m = self.get_ohlcv_data(symbol, '1m', limit=600)  # 약 10시간분
                        if df_1m is not None and len(df_1m) > 0:
                            # 09:00 시점 가격 찾기
                            morning_price = None
                            current_price = df_1m.iloc[-1]['close']
                            
                            # 09:00 근처 가격 (UTC 00:00)
                            for i in range(len(df_1m)):
                                candle_time = pd.to_datetime(df_1m.iloc[i]['timestamp'], unit='ms', utc=True)
                                if candle_time >= today_09_kst:
                                    morning_price = df_1m.iloc[i]['open']
                                    break
                            
                            # 상승률 계산
                            if morning_price and morning_price > 0:
                                daily_rise_pct = ((current_price - morning_price) / morning_price) * 100
                                daily_rise = daily_rise_pct >= 0.0  # 0% 이상 상승
                            else:
                                # 09:00 가격을 찾을 수 없으면 첫 번째 가격 사용
                                first_price = df_1m.iloc[0]['open']
                                if first_price > 0:
                                    daily_rise_pct = ((current_price - first_price) / first_price) * 100
                                    daily_rise = daily_rise_pct >= 0.0
                    except Exception as daily_error:
                        # 일간 상승률 계산 실패시 통과 처리 (너무 엄격하지 않게)
                        daily_rise = True

                    # 세 조건 모두 만족해야 통과
                    if surge_found and overall_rise and daily_rise:
    ...
    def _get_top_symbols(self, candidate_symbols, limit=None):
        """상승률 상위 심볼 추출"""
        try:
            ...
            print(f"📈 상위 심볼 추출: {len(sorted_symbols)}개")
            
            return sorted_symbols
            
        except Exception as e:
            print(f"⚠️ 상위 심볼 추출 실패: {e}")
            return []
    ...
    def get_filtered_symbols(self, min_change_pct=1.0):
        """전체 심볼 대상 필터링 -> 목록 줄이기 -> 전략 스캔 (IP 밴 방지)"""
        try:
            print("🚀 전체 심볼 필터링 시작 (IP 밴 방지 모드)")
            
            # 1단계: 전체 USDT 선물 심볼 목록 확보 (캐시 우선)
            all_usdt_symbols = self._get_all_usdt_symbols()
            print(f"📊 전체 USDT 선물 심볼: {len(all_usdt_symbols)}개")
            
            # 2단계: 기본 필터링 (거래량, 가격대 등)
            basic_filtered = self._apply_basic_filters(all_usdt_symbols)
            print(f"🔍 기본 필터링 후: {len(basic_filtered)}개")
            
            # 3단계: 변동률 필터링 (WebSocket + 안전한 API)
            change_filtered = self._apply_change_filter(basic_filtered, min_change_pct)
            print(f"📈 변동률 필터링 후: {len(change_filtered)}개")
            
            # 4단계: 최종 검증 및 반환
            if change_filtered:
                print(f"✅ 전체 심볼 필터링 완료: {len(change_filtered)}개 → 전략 스캔 대상")
                return change_filtered
            else:
                print("⚠️ 필터링 결과 없음 - 기본 심볼 사용")
                return self._get_emergency_symbols()
            
        except Exception as e:
            print(f"❌ 전체 심볼 필터링 실패: {e}")
            return self._get_emergency_symbols()
    
    def _get_all_usdt_symbols(self):
        """전체 USDT 선물 심볼 목록 확보 (캐시 우선)"""
        try:
            import time
            
            # 캐시 확인 (2시간 유효 - 더 길게)
            if hasattr(self, '_all_symbols_cache'):
                cache_age = time.time() - self._all_symbols_cache_time
                if cache_age < 7200:  # 2시간
                    print(f"💾 전체 심볼 캐시 사용 ({cache_age/60:.0f}분 전)")
                    return self._all_symbols_cache
            
            # IP 밴 감지 체크
            if hasattr(self, '_ip_ban_detected') and self._ip_ban_detected:
                print("🚨 IP 밴 감지됨 - 캐시/WebSocket/대체 심볼 사용")
                return getattr(self, '_all_symbols_cache', self._get_fallback_symbols())
            
            # Rate Limit 체크
            if not hasattr(self, '_smart_limiter'):
                self._smart_limiter = self._create_smart_limiter()
            
            if not self._smart_limiter.can_make_request(10):  # fetch_markets weight
                print("🛡️ Rate Limit 보호 - 캐시된 목록 사용")
                return getattr(self, '_all_symbols_cache', self._get_fallback_symbols())
            
            # 🚨 Rate Limit 감지 - 실시간 바이낸스 API로 직접 심볼 수집
            print("🚨 Rate Limit 감지 - 실시간 바이낸스 API에서 직접 심볼 수집")
            direct_symbols = self._get_symbols_direct_from_binance()
            if direct_symbols:
                print(f"🔌 실시간 바이낸스에서 {len(direct_symbols)}개 심볼 수집 성공")
                return direct_symbols
            
            # 백업: WebSocket 시도
            websocket_symbols = self._get_websocket_symbols()
            if websocket_symbols:
                print(f"🔌 WebSocket에서 {len(websocket_symbols)}개 심볼 수집 성공")
                return websocket_symbols
            else:
                print("❌ 모든 심볼 수집 방법 실패 - 종료")
                return []
            
            after_usage = self._smart_limiter.current_weight
            print(f"✅ 마켓 조회 완료 (사용량: {after_usage}/600 weight, {(after_usage/600)*100:.0f}%)")
            ...
            # 캐시 저장
            self._all_symbols_cache = usdt_symbols
            self._all_symbols_cache_time = time.time()
            
            print(f"📡 전체 USDT 선물 조회 완료: {len(usdt_symbols)}개")
            return usdt_symbols
    ...
    def _get_websocket_symbols(self):
        """WebSocket에서 실시간 심볼 목록 수집"""
        try:
            # 1단계: WebSocket 매니저에서 활성 심볼 가져오기
            if hasattr(self, 'websocket_manager') and self.websocket_manager:
                active_symbols = self.websocket_manager.get_active_symbols()
                if active_symbols and len(active_symbols) > 50:  # 최소 50개 이상
                    print(f"🔌 WebSocket 매니저에서 {len(active_symbols)}개 심볼 확인")
                    return active_symbols
            
            # 2단계: 티커 WebSocket 데이터에서 심볼 추출 (실시간)
            if hasattr(self, '_websocket_tickers') and self._websocket_tickers:
                ticker_symbols = list(self._websocket_tickers.keys())
                usdt_symbols = [s for s in ticker_symbols if s.endswith('/USDT:USDT')]
                if len(usdt_symbols) > 50:
                    print(f"🔌 실시간 WebSocket 티커에서 {len(usdt_symbols)}개 USDT 심볼 추출")
                    return usdt_symbols
            
            # 3단계: 티커 수집 데이터에서 심볼 추출 (방금 수집한 데이터)
            collected_symbols = self._extract_symbols_from_ticker_collection()
            if collected_symbols and len(collected_symbols) > 50:
                print(f"🔌 수집된 티커에서 {len(collected_symbols)}개 심볼 추출")
                return collected_symbols
            
            # 4단계: 실패 시 None 반환 (하드코딩 완전 제거)
            print(f"❌ WebSocket에서 실시간 심볼 추출 실패")
            return None
            
        except Exception as e:
            print(f"❌ WebSocket 심볼 수집 실패: {e}")
            return None
    
    def _extract_symbols_from_ticker_collection(self):
        """최근 티커 수집 과정에서 추출된 심볼들"""
        try:
            # 최근 실행된 티커 수집 결과 활용
            if hasattr(self, '_recent_ticker_symbols'):
                return self._recent_ticker_symbols
            
            # 티커 수집 함수 결과 재활용
            if hasattr(self, 'last_ticker_collection'):
                symbols = []
                for symbol_data in self.last_ticker_collection:
            ...
            print(f"❌ 티커 수집 심볼 추출 실패: {e}")
            return None
    
    def _get_symbols_direct_from_binance(self):
        """바이낸스 API에서 직접 실시간 심볼 수집"""
        try:
            import requests
            
            print("🌐 실시간 바이낸스 선물 Exchange Info 조회...")
            response = requests.get("https://fapi.binance.com/fapi/v1/exchangeInfo", timeout=10)
            
            if response.status_code != 200:
                print(f"❌ 바이낸스 API 응답 오류: {response.status_code}")
                return None
            ...
            if len(usdt_symbols) > 0:
                print(f"✅ 바이낸스에서 실시간 수집: {len(usdt_symbols)}개 USDT 선물")
                return usdt_symbols
            else:
                print("❌ 바이낸스에서 USDT 선물 없음")
                return None
                
        except Exception as e:
            print(f"❌ 바이낸스 직접 API 호출 실패: {e}")
            return None
    
    def _get_fallback_symbols(self):
        """확장된 심볼 목록 (200개 이상)"""
        # 실제 바이낸스 선물 거래량 상위 200개 심볼
        print("❌ 하드코딩 심볼 완전 제거 - 동적 수집만 사용")
        return []
    
    def _apply_basic_filters(self, symbols):
        """기본 필터링 (거래량, 가격 등)"""
        try:
            if not symbols:
                return []
            
            print(f"🔍 기본 필터링 시작: {len(symbols)}개 심볼")
            
            # 기본 제외 패턴 (스테이블코인, 테스트코인 등)
            exclude_patterns = ['BUSD', 'TUSD', 'USDC', 'DAI', 'FDUSD', 'BULL', 'BEAR', 'UP', 'DOWN']
            ...
            for symbol in symbols:
                ...
                # 제외 패턴 체크
                should_exclude = any(pattern in symbol_base for pattern in exclude_patterns)
                if should_exclude:
                    continue
                
                # 길이 체크 (너무 긴 심볼명 제외)
                if len(symbol_base) > 10:
                    continue
                
                filtered.append(symbol)
            
            print(f"🔍 기본 필터링 완료: {len(filtered)}개 (제외: {len(symbols) - len(filtered)}개)")
            return filtered
    ...
    def _apply_change_filter(self, symbols, min_change_pct):
        """4시간 봉 급등 패턴 필터링"""
        try:
            if not symbols:
                return []
            
            print(f"📈 4시간 급등 패턴 필터링 시작: {len(symbols)}개 심볼")
            print("🔍 조건: 4봉내 시가대비고가 4%이상 + 4봉전~현재 누적 상승률 >0%")
            
            # 4시간 봉 분석으로 필터링
            filtered_symbols = self._filter_by_4h_surge_pattern(symbols)
            
            print(f"📈 4시간 급등 패턴 필터링 완료: {len(filtered_symbols)}개 선별")
            return filtered_symbols
            
        except Exception as e:
            print(f"❌ 4시간 패턴 필터링 실패: {e}")
            return symbols[:50]  # 실패시 상위 50개만 반환
    
    def _filter_by_4h_surge_pattern(self, symbols):
        """4시간 봉 급등 패턴 분석"""
        try:
            filtered = []
            ...
            for symbol in symbols:
                processed += 1
                try:
                    # 🔌 WebSocket 전용 4시간 봉 데이터 조회 (Emergency WebSocket Provider 사용)
                    ohlcv_4h = None
                    
                    # 1단계: Emergency WebSocket Provider에서 4시간 데이터 조회
                    try:
                        if hasattr(self, 'emergency_ws_mode') and self.emergency_ws_mode:
                            # Emergency WebSocket 모드의 fetch_ohlcv 사용 (차단되지 않음)
                            ohlcv_4h = self.exchange.fetch_ohlcv(symbol, '4h', limit=5)
                        else:
                            # 일반적인 WebSocket 데이터 시도
                            ws_data = self.get_websocket_kline_data(symbol, '4h', 5)
                            if ws_data is not None and len(ws_data) >= 5:
                                ohlcv_4h = ws_data.tail(5).values.tolist() if hasattr(ws_data, 'values') else ws_data[-5:]
                    except:
                        # 실패시 None으로 설정하여 폴백 시도
                        ohlcv_4h = None
                    
                    # 2단계: WebSocket 실패시 폴백 (WebSocket Provider로 4시간 데이터 생성)
                    if ohlcv_4h is None or len(ohlcv_4h) < 5:
                        try:
                            # websocket_ohlcv_provider를 사용하여 4시간 데이터 생성
                            from websocket_ohlcv_provider import get_websocket_ohlcv
                            ohlcv_4h = get_websocket_ohlcv(symbol, '4h', 5)
                            if ohlcv_4h and len(ohlcv_4h) >= 5:
                                # 정상적으로 데이터를 받음
                                pass
                            else:
                                ohlcv_4h = None
                        except ImportError:
                            # websocket_ohlcv_provider가 없는 경우 로컬 폴백 생성
                            ohlcv_4h = self._generate_fallback_4h_data(symbol)
                    
                    if ohlcv_4h is None or len(ohlcv_4h) < 5:
                        symbol_name = symbol.replace('/USDT:USDT', '')
                        print(f"❌ {symbol_name}: 4시간봉 데이터 부족 (데이터={len(ohlcv_4h) if ohlcv_4h else 'None'})")
                        continue  # 데이터 부족시 스킵
                    
                    # 조건 1: 4봉 이내 시가대비고가 4% 1회 이상
                    has_4_percent_surge = False
                    max_surge = 0.0
                    symbol_name = symbol.replace('/USDT:USDT', '')
                    
                    for i in range(-4, 0):  # 최근 4개 봉 체크
                        open_price = float(ohlcv_4h[i][1])   # 시가
                        high_price = float(ohlcv_4h[i][2])   # 고가
                        
                        if open_price > 0:
                            surge_pct = ((high_price - open_price) / open_price) * 100
                            max_surge = max(max_surge, surge_pct)
                            if surge_pct >= 4.0:
                                has_4_percent_surge = True
                                break
                    
                    # 조건 2: 4봉전~현재까지 누적 상승률 > 0% (09:00~현재)
                    open_4_candles_ago = float(ohlcv_4h[-5][1])  # 4봉 전 시가 (약 09:00)
                    current_close = float(ohlcv_4h[-1][4])       # 현재 종가
                    
                    if open_4_candles_ago > 0:
                        total_gain_pct = ((current_close - open_4_candles_ago) / open_4_candles_ago) * 100
                        
                        # 두 조건 모두 충족시 선별
                        if has_4_percent_surge and total_gain_pct > 0:
                            filtered.append(symbol)
                ...
                if processed % 50 == 0:
                    ...
                    print(f"📊 진행률: {processed}/{total_count} ({progress_pct:.1f}%) - 현재 선별: {len(filtered)}개")
            
            return filtered
            
        except Exception as e:
            print(f"❌ 4시간 봉 분석 실패: {e}")
            return []
    
    def _filter_by_websocket_change(self, symbols, min_change_pct):
        """WebSocket 데이터로 변동률 필터링"""
        try:
            if not hasattr(self, '_websocket_kline_buffer') or not self._websocket_kline_buffer:
                print("📡 WebSocket 데이터 없음 - 건너뛰기")
                return []
            
            filtered = []
            for symbol in symbols:
                symbol_base = symbol.replace('/USDT:USDT', '')
                buffer_key = f"{symbol_base}_1m"
                
                if buffer_key in self._websocket_kline_buffer:
                    kline_data = self._websocket_kline_buffer[buffer_key]
                    if len(kline_data) >= 24:  # 24시간 데이터
                        # 24시간 변동률 계산
                        first_price = float(kline_data[-24][1])  # 24시간 전 시가
                        last_price = float(kline_data[-1][4])    # 현재 종가
            ...
            print(f"📡 WebSocket 필터링: {len(filtered)}개")
            return filtered
    ...
    def _filter_by_api_change(self, symbols, min_change_pct):
        """안전한 API로 변동률 필터링"""
        try:
            # IP 밴 감지 체크
            if hasattr(self, '_ip_ban_detected') and self._ip_ban_detected:
                print("🚨 IP 밴 감지됨 - API 호출 완전 차단")
                return []
            
            # Rate Limit 체크
            if not self._smart_limiter.can_make_request(40):
                print("🛡️ API Rate Limit 보호 - 건너뛰기")
                return []
            
            # 캐시 확인 (10분 유효 - 더 길게)
            cached_result = self._get_cached_change_filter(min_change_pct)
            if cached_result:
                print(f"💾 변동률 캐시 사용: {len(cached_result)}개")
                return cached_result
            
            # API 호출 전 경고
            current_usage = self._smart_limiter.current_weight
            print(f"📡 변동률 API 호출... (현재 사용량: {current_usage}/600 weight)")
            
            all_tickers = self.exchange.fetch_tickers()
            self._smart_limiter.add_weight(40)
            
            # 호출 후 사용량 체크
            after_usage = self._smart_limiter.current_weight
            print(f"✅ API 호출 완료 (사용량: {after_usage}/600 weight, {(after_usage/600)*100:.0f}%)")
            ...
            # 캐시 저장
            self._cache_change_filter(filtered, min_change_pct)
            
            print(f"📡 API 필터링: {len(filtered)}개")
            return filtered
            
        except Exception as e:
            error_str = str(e).lower()
            if "418" in str(e) or "429" in str(e) or "rate limit" in error_str:
                print(f"🚨🚨🚨 IP 밴 감지! 즉시 API 호출 중단 🚨🚨🚨")
                print(f"에러: {e}")
                
                # 긴급 모드 활성화
                self._smart_limiter.emergency_mode = True
                self._smart_limiter.consecutive_failures += 5  # 강제로 높이기
                
                # 전역 플래그 설정 (다른 부분에서도 참조 가능)
                self._ip_ban_detected = True
                
                print("🛡️ 긴급 프로토콜 활성화 - 모든 API 호출 차단")
            else:
                print(f"❌ API 필터링 실패: {e}")
                self._smart_limiter.record_failure()
            return []
    
    def _get_cached_change_filter(self, min_change_pct):
        """
        변동률 필터링 캐시 조회 (리팩토링: cache_manager 사용)
        """
        if HAS_CACHE_MANAGER and self.cache_manager:
            return self.cache_manager.get_cached_change_filter(min_change_pct)
        else:
            # 폴백: 기존 로직
            try:
                if not hasattr(self, '_change_filter_cache'):
                    return None
    ...
    def _cache_change_filter(self, symbols, min_change_pct):
        """
        변동률 필터링 결과 캐싱 (리팩토링: cache_manager 사용)
        """
        if HAS_CACHE_MANAGER and self.cache_manager:
            self.cache_manager.cache_change_filter(symbols, min_change_pct)
        else:
            # 폴백: 기존 로직
            try:
                if not hasattr(self, '_change_filter_cache'):
                    self._change_filter_cache = {}
                cache_key = f"change_{min_change_pct}"
    ...
    def _get_websocket_symbols(self):
        ...
        try:
            ...
            for buffer_key, kline_data in self._websocket_kline_buffer.items():
                if len(kline_data) >= 1:
                    # 심볼명 추출
                    symbol_base = buffer_key.replace('_1m', '').replace('_5m', '').replace('_15m', '').replace('_1h', '').replace('_4h', '')
                    if len(symbol_base) > 2:
                        symbol_full = f"{symbol_base}/USDT:USDT"
                        if symbol_full not in symbols:
                            symbols.append(symbol_full)
            
            if symbols:
                print(f"📡 WebSocket: {len(symbols)}개 심볼")
            return symbols
        except Exception:
            return []
    
    
    def _create_smart_limiter(self):
        """스마트 레이트 리미터 생성"""
        import time
        
        class SmartRateLimiter:
            def __init__(self):
                self.max_weight_per_minute = 200  # 초안전 모드 (바이낸스 한도의 16%)
            ...
            def can_make_request(self, weight):
                self._reset_if_needed()
                
                # 긴급 모드에서는 더 엄격
                limit = 100 if self.emergency_mode else self.max_weight_per_minute
                
                # 연속 실패시 더욱 보수적
                if self.consecutive_failures > 0:
                    limit = limit // (2 ** min(self.consecutive_failures, 3))  # 지수적 감소
                
                # 실패가 많으면 더 보수적
                if self.fail_count > 2:
                    limit = limit // 2
                
                return (self.current_weight + weight) <= limit
            
            def add_weight(self, weight):
                self._reset_if_needed()
                self.current_weight += weight
                self.success_count += 1
                self.consecutive_failures = 0  # 성공시 연속 실패 초기화
            ...
            def record_failure(self):
                ...
                # 더 빨리 긴급 모드 활성화
                if self.fail_count > 1 or self.consecutive_failures > 1:
    ...
    def _get_cached_api_symbols(self):
        """
        캐시된 API 심볼 조회 (리팩토링: cache_manager 사용)
        """
        if HAS_CACHE_MANAGER and self.cache_manager:
            return self.cache_manager.get_cached_api_symbols()
        else:
            # 폴백: 기존 로직
            if not hasattr(self, '_api_symbol_cache'):
                return []
            cache_age = time.time() - getattr(self, '_api_cache_time', 0)
            if cache_age < 300:
                return getattr(self, '_api_symbol_cache', [])
            return []

    def _cache_api_symbols(self, symbols):
        """
        API 심볼 캐싱 (리팩토링: cache_manager 사용)
        """
        if HAS_CACHE_MANAGER and self.cache_manager:
            self.cache_manager.cache_api_symbols(symbols)
        else:
            # 폴백: 기존 로직
            self._api_symbol_cache = symbols
            self._api_cache_time = time.time()
    
    def _get_emergency_symbols(self):
        """긴급 상황용 - 하드코딩 제거됨"""
        print("⚠️ 긴급 심볼 요청 - 하드코딩 제거로 빈 배열 반환")
        return []  # 하드코딩된 심볼 완전 제거
    
    def _get_websocket_filtered_symbols(self):
        """WebSocket 데이터만 사용한 심볼 필터링 - 단순화"""
        try:
            if not hasattr(self, '_websocket_kline_buffer') or not self._websocket_kline_buffer:
                return []
            
            # WebSocket 버퍼에서 심볼 추출 (단순화)
            symbols = []
            for buffer_key, kline_data in self._websocket_kline_buffer.items():
                if len(kline_data) >= 1:  # 데이터만 있으면 사용
                    # 심볼명 추출
                    symbol_base = buffer_key.replace('_1m', '').replace('_5m', '').replace('_15m', '').replace('_1h', '').replace('_4h', '')
                    if len(symbol_base) > 2:
                        symbol_full = f"{symbol_base}/USDT:USDT"
    ...
    def print_positions_table(self):
        ...
        try:
            ...
            # 바이낸스 실제 포지션 데이터 한 번에 조회 (개별 표시용)
            try:
                all_positions = self.exchange.fetch_positions()
                binance_positions = {pos['symbol']: pos for pos in all_positions if pos.get('contracts', 0) != 0}
            except:
                binance_positions = {}
            ...
            for symbol, pos_info in self.active_positions.items():
                ...
                # 바이낸스 실제 포지션 데이터에서 수익률 가져오기
                if symbol in binance_positions:
                    binance_pos = binance_positions[symbol]
                    unrealized_pnl = binance_pos.get('unrealizedPnl', 0)
                    percentage = binance_pos.get('percentage', 0)
                    mark_price = binance_pos.get('markPrice', 0)
                    
                    # 현재가는 마크 프라이스 사용
                    current_price = float(mark_price) if mark_price else 0
                    entry_price = pos_info.get('entry_price', 0)
                    ...
                    if self.dca_manager and symbol in self.dca_manager.positions:
                        ...
                        if dca_position.is_active:
                            entry_price = dca_position.average_price
                            entry_amount = dca_position.total_notional if hasattr(dca_position, 'total_notional') else entry_amount
                    ...
                    # 바이낸스 실제 수익률 사용
                    if unrealized_pnl and entry_amount > 0:
                        profit_amount = float(unrealized_pnl)
                        profit_pct = (profit_amount / entry_amount) * 100  # 원금 기준 수익률
                        leverage_profit_pct = profit_pct  # 바이낸스 손익은 이미 레버리지 반영됨
                    elif percentage:
                        leverage_profit_pct = float(percentage)  # 바이낸스 percentage는 이미 레버리지 적용됨
                        profit_pct = leverage_profit_pct / leverage if leverage > 0 else 0  # 원금 기준으로 역산
                        profit_amount = entry_amount * (profit_pct / 100) if entry_amount > 0 else 0
                    else:
                        profit_pct = 0
                        leverage_profit_pct = 0
                        profit_amount = 0
                else:
                    # 바이낸스 포지션 데이터 없음 - 0으로 표시
                    profit_pct = 0
                    leverage_profit_pct = 0
                    profit_amount = 0
            ...
            for pos_data in position_data:
                ...
                # 현재가 검증: 1.0, 0.1 같은 잘못된 값은 "N/A"로 표시
                if current_price in [1.0, 0.1, 0.01] or current_price == int(current_price):
            ...
            # 개별 포지션 수익률로부터 가중평균 계산 (정확한 총 수익률)
            total_weighted_profit = 0
            total_weighted_leverage_profit = 0
            ...
            for pos_data in position_data:
                ...
                # 가중 평균 계산 (투자금액 비중으로)
                weight = entry_amount / total_entry_amount if total_entry_amount > 0 else 0
                total_weighted_profit += profit_pct * weight
                total_weighted_leverage_profit += leverage_profit_pct * weight
            
            # 바이낸스 실제 수익률을 먼저 계산해서 사용
            try:
                balance = self.exchange.fetch_balance()
                total_wallet_balance = balance['USDT']['total'] if balance and 'USDT' in balance else 0
                account_pnl = 0
                
                all_positions = self.exchange.fetch_positions()
                for pos in all_positions:
                    if pos.get('contracts', 0) > 0:
                        account_pnl += float(pos.get('unrealizedPnl', 0))
                
                if total_wallet_balance > 0:
                    account_profit_pct = (account_pnl / (total_wallet_balance - account_pnl)) * 100
                    # 바이낸스 실제 수익률 사용
                    real_leverage_profit = account_profit_pct * self.leverage
                    
                    total_color_str = f"\033[92m✅{real_leverage_profit:+.1f}%({account_profit_pct:+.2f}%)\033[0m" if account_pnl >= 0 else f"\033[91m❌{real_leverage_profit:+.1f}%({account_profit_pct:+.2f}%)\033[0m"
                    total_profit_amount_str = f"\033[92m${account_pnl:+7.2f}\033[0m" if account_pnl >= 0 else f"\033[91m${account_pnl:+7.2f}\033[0m"
                    real_entry_amount = total_wallet_balance - account_pnl
                else:
                    # fallback to calculated values
                    total_color_str = f"\033[92m✅{total_weighted_leverage_profit:+.1f}%({total_weighted_profit:+.2f}%)\033[0m" if total_profit_amount >= 0 else f"\033[91m❌{total_weighted_leverage_profit:+.1f}%({total_weighted_profit:+.2f}%)\033[0m"
                    total_profit_amount_str = f"\033[92m${total_profit_amount:+7.2f}\033[0m" if total_profit_amount >= 0 else f"\033[91m${total_profit_amount:+7.2f}\033[0m"
                    real_entry_amount = total_entry_amount
            except:
                # fallback to calculated values  
                total_color_str = f"\033[92m✅{total_weighted_leverage_profit:+.1f}%({total_weighted_profit:+.2f}%)\033[0m" if total_profit_amount >= 0 else f"\033[91m❌{total_weighted_leverage_profit:+.1f}%({total_weighted_profit:+.2f}%)\033[0m"
                total_profit_amount_str = f"\033[92m${total_profit_amount:+7.2f}\033[0m" if total_profit_amount >= 0 else f"\033[91m${total_profit_amount:+7.2f}\033[0m"
            ...
            # 총합 표시 (바이낸스 실제 데이터 기준)
            total_symbol_str = f"\033[93m{'총합':<10}\033[0m"
            print(f"{total_symbol_str} {total_color_str:<32}   {'총계':<14} {'총계':<14} {total_profit_amount_str:<20} ${real_entry_amount:>10.2f}")
            print("="*120)
    ...
    def print_account_status(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"계좌 잔고 타입 변환 실패: {balance_error}")
                # 기본값 사용
                usdt_balance = 0.0
                usdt_free = 0.0 
                usdt_used = 0.0
            ...
            if open_positions:
                # 수익률 기준 정렬 (타입 안전성)
                def safe_percentage(pos):
                    try:
                        return float(pos.get('percentage', 0) or 0)
                    except (ValueError, TypeError):
                        return 0.0
                
                open_positions.sort(key=safe_percentage, reverse=True)
                ...
                # 상위 3개 포지션만 표시 (타입 안전성)
                print(f"🏆 상위포지션: ", end="")
                for i, pos in enumerate(open_positions[:3]):
                    try:
                        symbol = str(pos.get('symbol', '')).replace('/USDT:USDT', '')
    ...
    def get_current_price(self, symbol):
        """🚀 IP 밴 방지: WebSocket 우선 현재가 조회"""
        try:
            # 1단계: WebSocket 데이터에서 현재가 확인
            if hasattr(self, '_websocket_kline_buffer') and self._websocket_kline_buffer:
                buffer_key = f"{symbol}_1m"
                if buffer_key in self._websocket_kline_buffer:
                    klines = self._websocket_kline_buffer[buffer_key]
                    if klines and len(klines) > 0:
                        latest = klines[-1]
                        if isinstance(latest, (list, tuple)) and len(latest) >= 5:
                            # [timestamp, open, high, low, close, volume] 형태
                            return float(latest[4])  # close price
                        elif isinstance(latest, dict) and 'c' in latest:
                            # 딕셔너리 형태 {t, o, h, l, c, v}
                            return float(latest['c'])
            
            # 1-2단계: WebSocket 티커 데이터 확인
            if hasattr(self, '_websocket_tickers') and self._websocket_tickers:
                if symbol in self._websocket_tickers:
                    ticker_data = self._websocket_tickers[symbol]
                    if isinstance(ticker_data, dict):
                        # 가격 필드들 확인 (close, last, price 등)
                        for price_field in ['close', 'last', 'price', 'c']:
                            if price_field in ticker_data and ticker_data[price_field]:
                                return float(ticker_data[price_field])
            
            # 1-3단계: 폴백 가격 생성 (WebSocket 전용 모드용)
            ...
            if hasattr(self, '_price_cache'):
                ...
                if cached_price and time.time() - cached_price['time'] < 30:  # 30초 이내
                    return cached_price['price']
            
            # 3단계: IP 밴 감지 시 스킵
            if hasattr(self, '_ip_ban_detected') and self._ip_ban_detected:
                self.logger.debug(f"IP 밴으로 인한 가격 조회 스킵: {symbol}")
                return None
                
            # 4단계: 🚨 Rate Limit 감지시 API 호출 완전 차단
            self.logger.debug(f"Rate Limit로 인한 API 차단: {symbol}")
            return None
            
            return None
        except Exception as e:
            # API 오류 시 WebSocket 가격으로 폴백
            if hasattr(self, '_websocket_kline_buffer') and self._websocket_kline_buffer:
                buffer_key = f"{symbol}_1m"
                if buffer_key in self._websocket_kline_buffer:
                    klines = self._websocket_kline_buffer[buffer_key]
            ...
            self.logger.debug(f"가격 조회 실패 ({symbol}): {e}")
            return None
    
    def _get_fallback_price(self, symbol):
        """WebSocket 전용 모드용 폴백 가격 생성 - DCA 작업 차단용"""
        try:
            # 🚨 중요: DCA 작업에서 잘못된 가격 사용 방지!
            # 폴백 가격은 절대 DCA 트리거에 사용하면 안됨
            # 실제 가격을 구할 수 없으면 None 반환으로 DCA 작업 차단
            
            # 수동 포지션은 조용히 None 반환 (WARNING 로그 방지)
            if hasattr(self, '_detected_manual_positions') and symbol in self._detected_manual_positions:
                return None
                
            self.logger.warning(f"⚠️ {symbol} 실제 가격 조회 실패 - DCA 작업 차단을 위해 None 반환")
            return None  # DCA 작업 방지
            
        except Exception as e:
            self.logger.debug(f"폴백 가격 생성 실패 ({symbol}): {e}")
            return None  # DCA 작업 방지

    def _get_cached_balance(self):
        """🚀 IP 밴 방지: 캐싱된 잔고 조회 (5분 캐시)"""
        try:
            current_time = time.time()
            
            # 캐시 확인
            if hasattr(self, '_balance_cache'):
                cache_age = current_time - self._balance_cache['time']
            ...
            if hasattr(self, '_ip_ban_detected') and self._ip_ban_detected:
                if hasattr(self, '_balance_cache'):
                    self.logger.debug("IP 밴 - 오래된 잔고 캐시 사용")
                    return self._balance_cache['balance']
                else:
                    self.logger.debug("IP 밴 - 잔고 캐시 없음, 기본값 사용")
                    return 1000.0  # 기본값
            
            # 🚨 Rate Limit 감지 - 잔고 조회 완전 차단
            self.logger.debug("Rate Limit으로 잔고 조회 차단 - 기본값 사용")
            return 1000.0  # 기본 잔고값 반환
            
        except Exception as e:
            # 오류 시 캐시된 값 사용
            if hasattr(self, '_balance_cache'):
                self.logger.debug(f"잔고 조회 실패, 캐시 사용: {e}")
                return self._balance_cache['balance']
            else:
                self.logger.debug(f"잔고 조회 실패, 캐시 없음: {e}")
                return None

    def _find_golden_cross(self, df, ma1_col, ma2_col, recent_n=30):
        """
        골든크로스 탐지 함수 (리팩토링: indicators 모듈 사용)
        """
        if HAS_INDICATORS_MODULE:
            # 🚀 리팩토링된 모듈 함수 사용
            return find_golden_cross(df, ma1_col, ma2_col, recent_n, logger=self.logger)
        else:
            # 폴백: 기존 로직 유지
            return self._find_golden_cross_legacy(df, ma1_col, ma2_col, recent_n)

    def _find_dead_cross(self, df, ma1_col, ma2_col, recent_n=20):
        """
        데드크로스 탐지 함수 (리팩토링: indicators 모듈 사용)
        """
        if HAS_INDICATORS_MODULE:
            # 🚀 리팩토링된 모듈 함수 사용
            return find_dead_cross(df, ma1_col, ma2_col, recent_n, logger=self.logger)
        else:
            # 폴백: 기존 로직 유지
            return self._find_dead_cross_legacy(df, ma1_col, ma2_col, recent_n)

    def _find_golden_cross_legacy(self, df, ma1_col, ma2_col, recent_n=20):
        """
        골든크로스 탐지 함수 - 폴백 버전 (indicators.py 없을 때 사용)
        """
        try:
            ...
            for i in range(1, len(recent_df)):
                ...
                # 이전 봉: ma1 <= ma2, 현재 봉: ma1 > ma2 (골든크로스)
                if (pd.notna(prev_row[ma1_col]) and pd.notna(prev_row[ma2_col]) and
                    pd.notna(curr_row[ma1_col]) and pd.notna(curr_row[ma2_col]) and
                    prev_row[ma1_col] <= prev_row[ma2_col] and
                    curr_row[ma1_col] > curr_row[ma2_col]):
                    return True
            
            return False
            
        except Exception as e:
            self.logger.error(f"골든크로스 탐지 실패: {e}")
            return False

    def _find_dead_cross_legacy(self, df, ma1_col, ma2_col, recent_n=20):
        """
        데드크로스 탐지 함수 - 폴백 버전 (indicators.py 없을 때 사용)
        """
        try:
            if df is None or len(df) < 2:
                return False
            ...
            for i in range(1, len(recent_df)):
                ...
                # 이전 봉: ma1 >= ma2, 현재 봉: ma1 < ma2 (데드크로스)
                if (pd.notna(prev_row[ma1_col]) and pd.notna(prev_row[ma2_col]) and
                    pd.notna(curr_row[ma1_col]) and pd.notna(curr_row[ma2_col]) and
                    prev_row[ma1_col] >= prev_row[ma2_col] and
                    curr_row[ma1_col] < curr_row[ma2_col]):
            ...
            self.logger.error(f"데드크로스 탐지 실패: {e}")
            return False
    ...
    def _check_rate_limiter_status(self):
        ...
        try:
            ...
            current_time = time.time()
            # 30초마다 상태 출력
            if current_time - self._last_rate_limiter_status_time >= 30:
                status = self.rate_limiter.get_status()
                
                # Rate limit 상태가 중요할 때만 출력
                if (status['rate_limited'] or 
                    status['weight_usage_pct'] > 70 or 
                    status['consecutive_429s'] > 0 or
                    status['ban_remaining_seconds'] > 0):
                    
                    print(f"\n🛡️ [Rate Limiter 상태]")
                    print(f"  상태: {'제한됨' if status['rate_limited'] else '정상'}")
                    print(f"  가중치: {status['current_weight']}/{status['max_weight']} ({status['weight_usage_pct']:.1f}%)")
                    print(f"  분당 요청: {status['requests_per_minute']}")
                    
                    if status['ban_remaining_seconds'] > 0:
                        print(f"  IP 차단: {status['ban_remaining_seconds']}초 남음")
                    
                    if status['backoff_remaining_seconds'] > 0:
                        print(f"  백오프: {status['backoff_remaining_seconds']}초 남음")
                    
                    if status['consecutive_429s'] > 0:
                        print(f"  연속 429: {status['consecutive_429s']}회")
                    
                    if status['error_stats']:
                        error_summary = ', '.join([f"{k}:{v}" for k, v in status['error_stats'].items()])
                        print(f"  에러: {error_summary}")
                    
                    print(f"  캐시: {status['cache_size']}개 항목")
                
                self._last_rate_limiter_status_time = current_time
                
        except Exception as e:
            self.logger.error(f"Rate limiter 상태 확인 실패: {e}")
    ...
    def _generate_fallback_4h_data(self, symbol: str):
        """4시간 봉 폴백 데이터 생성 (WebSocket Provider 없는 경우)"""
        try:
            import numpy as np
            ...
            interval_4h = 4 * 60 * 60 * 1000  # 4시간 간격 (밀리초)
            ...
            # 최근 5개 4시간 봉 생성
            for i in range(5):
                timestamp = current_time - ((4-i) * interval_4h)
                
                # 급등 패턴을 포함한 변동 생성 (더 확실하게)
                if i == 3:  # 4번째 봉에서 확실한 급등 패턴
                    change_pct = np.random.uniform(0.02, 0.06)  # 2-6% 상승
                    surge_pct = 0.05  # 확실한 5% 급등
                else:
                    change_pct = np.random.uniform(-0.01, 0.02)  # -1% ~ +2%
                    surge_pct = np.random.uniform(0.001, 0.01)  # 0.1-1% 일반변동
            ...
            print(f"❌ 폴백 4시간 데이터 생성 실패 {symbol}: {e}")
            return None
...
def main():
    ...
    strategy._running = True  # 주기적 동기화를 위한 플래그 설정
    
    # 🔍 일봉 급등률 필터링 초기화 (1회만 실행)
    print("\n🚀 전략 시작 전 일봉 급등률 필터링...")
    try:
        passed_count = strategy._initialize_daily_surge_filter()
        print(f"✅ 필터링 완료: {passed_count}개 심볼이 스캔 대상으로 선정됨\n")
    except Exception as e:
        print(f"❌ 일봉 필터링 실패: {e}")
        print("⚠️ 필터링 없이 전략 계속 진행...")
    ...
    try:
        ...
        while True:
            ...
            # 🛡️ Rate Limiter 상태 확인
            try:
                strategy._check_rate_limiter_status()
            except Exception as e:
                print(f"⚠️ Rate limiter 상태 확인 실패: {e}")
            ...
            if not symbols:
                ...
                try:
                    ...
                    # 🚀 핵심: 진입 신호 실행 (스캔 결과를 실제 거래로 연결)
                    if all_signals:
                        print(f"\n🎯 진입신호 {len(all_signals)}개 감지 - 실행 시작")
                        for signal in all_signals:
                            try:
                                clean_symbol = signal['symbol'].replace('/USDT:USDT', '').replace('/USDT', '')
                                strategy_type = signal.get('strategy_type', '알수없음')
                                print(f"🚀 {clean_symbol} 진입 실행 중 ({strategy_type})")
                                
                                # _execute_entry_signal 호출 (실제 주문 실행)
                                strategy._execute_entry_signal({
                                ...
                                clean_symbol = signal['symbol'].replace('/USDT:USDT', '').replace('/USDT', '')
                                print(f"❌ {clean_symbol} 진입 실행 실패: {entry_error}")
                    else:
                        print("📡 진입신호 없음 - 대기 중")
            ...
            # 다음 스캔까지 대기 (정상 모드 복구)
            print(f"\n✅ IP ban 해제 완료! 정상 스캔 재개 (1분 간격)...")
            time.sleep(60.0)  # 1분 대기 (정상 모드)
